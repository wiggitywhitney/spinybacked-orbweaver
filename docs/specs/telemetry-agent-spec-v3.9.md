...
2. **File content heuristic** (fallback) — If `package.json` is unreadable (e.g., malformed JSON) or unavailable (e.g., `projectDir` not provided to `updateSdkInitFile`), scan the file content for `import` declarations or `export` keywords using simple string matching. Presence of either keyword indicates ESM; absence indicates CJS. This detection is not AST-based and may produce false positives from keywords in comments or strings — acceptable for a fallback that rarely fires in practice, since `package.json` must exist per init requirements.

Note: Node.js also supports per-file module system overrides via file extension (`.mjs` = ESM, `.cjs` = CJS regardless of `package.json`). The current implementation does not check file extensions — the PoC targets `**/*.js` files where the `package.json` `"type"` field is the governing signal. Extension-based detection could be added if the agent's file discovery scope expands to include `.mjs`/`.cjs` files.
...
**Failure classification:** Not all failures are worth retrying. The fix loop classifies `instrumentFile` failures into two categories that determine whether the Coordinator retries or moves on immediately:

| Category | Failure Type | Description | Rationale |
|----------|-------------|-------------|-----------|
| **Retryable** | Elision detected | Agent output contains placeholder comments (`// ...`, `// existing code`, `/* ... */`) instead of the complete file | Transient LLM failure — the model was lazy on this attempt but may produce the full file on retry. Elision is the single most common agent failure mode. |
| **Retryable** | Null parsed output | The API returned a response but `parsed_output` was null (structured output parsing failed) | Transient — the model produced output that didn't match the Zod schema. A retry with the same prompt often succeeds. |
| **Retryable** | Validation failures | Tier 1 (syntax, lint, Weaver check) or Tier 2 (semantic quality) failures from the validation chain | The fix loop exists precisely to address these — the agent receives structured error feedback and attempts correction. Subject to early exit heuristics (monotonicity, duplicate detection). |
| **Terminal** | Token budget exceeded | Cumulative token usage across all attempts exceeds `maxTokensPerFile` | Budget is exhausted — further attempts would exceed the configured cost ceiling. No amount of retrying changes the file's token economics. |
| **Terminal** | File unreadable | Source file cannot be read from disk (permissions, deleted, encoding errors) | Deterministic infrastructure failure — retrying the same `fs.readFile` call will produce the same error. |
| **Terminal** | Deterministic parse failure | Structured output parsing fails deterministically on the same input (e.g., response too large for output schema, malformed JSON that no retry will fix) | Unlike null parsed output (transient), a deterministic parse failure reproduces on every attempt. The distinction is made after the first retry: if the same parse error recurs, it's reclassified as terminal via duplicate error detection. |
| **Terminal** | API authentication failure | Anthropic API returns 401/403 (invalid or expired API key) | Deterministic — the key won't become valid on retry. Affects all subsequent files too, so the Coordinator should abort the entire run, not just this file. |
| **Terminal** | Network failure | Anthropic API is unreachable (DNS, timeout, connection refused) | The fix loop does not retry network errors — retry handling belongs in the HTTP client (bounded retries with exponential backoff). If client-level retries are exhausted, the Coordinator marks the file as failed without entering the fix loop. |

**Implementation coupling:** The `isRetryableInstrumentError` function in the fix loop classifies errors by matching substrings in `instrumentFile` error messages (`"null parsed_output"`, `"elision detected"`). Validation failures don't flow through this path — they're always retryable by design (that's what the fix loop does). The terminal cases either prevent `instrumentFile` from being called at all (file unreadable), or surface as exceptions that the Coordinator catches and maps to `status: "failed"` without entering the retry loop.
...
**`@opentelemetry/api` is always a peerDependency** regardless of strategy. The OTel JS contrib GUIDELINES.md mandates this — multiple instances in `node_modules` cause silent trace loss via no-op fallbacks. The dependency strategy only affects **instrumentation packages** discovered and installed by the agent (e.g., `@opentelemetry/instrumentation-pg`, `@traceloop/instrumentation-anthropic`). The agent does not install or modify SDK packages (`@opentelemetry/sdk-node`, `@opentelemetry/sdk-trace-base`, etc.) — those are the user's responsibility (see Prerequisites).

**For distributable packages (`dependencyStrategy: peerDependencies`):** The SDK package (`@opentelemetry/sdk-node`) must also be a peerDependency, not a direct dependency. The deployer chooses the SDK version, same rationale as the API — a library should not force a specific SDK version on its consumers. If the user's SDK init file imports `@opentelemetry/sdk-node` and it is currently in `dependencies`, the Coordinator should flag this in the PR summary as a recommendation to move it to `peerDependencies`. The agent does not make this change automatically (it only manages instrumentation packages, not SDK packages).
