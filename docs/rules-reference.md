...
- **Tier 1 (Structural gates)**: Binary pass/fail prerequisites. If any gate fails, the instrumented file is reverted to its original state — the agent broke something fundamental. Tier 1 checks are dispatched through `src/validation/chain.ts`: ELISION lives in `src/validation/tier1/elision.ts`, WEAVER in `src/validation/tier1/weaver.ts`, and NDS-001 (syntax) / LINT are dispatched through the language provider (`provider.checkSyntax()` / `provider.lintCheck()`), with implementations in `src/languages/javascript/validation.ts` and the TypeScript equivalent.
...
### SCH-002 auto-registration (2026-06-04)

[PRD #902](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/902) added deterministic auto-registration of novel `setAttribute()` keys as a reliability backstop. No rule IDs changed; SCH-002 behavior changed: novel attribute keys present in instrumented code but missing from `schemaExtensions` are now registered before SCH-002 fires, rather than causing a blocking failure. See the SCH-002 entry in Tier 2 blocking rules for the full pipeline description.
...
- **SCH-002 rebuilt**: Extension acceptance now validates that declared attribute extensions are not semantic duplicates of existing registry attributes. Extension acceptance uses normalization (delimiter-variant detection, e.g., `http_request_duration` ≈ `http.request.duration`) and an optional LLM judge — Jaccard is disabled for acceptance to avoid false positives on legitimate sibling attributes. A type-compatibility pre-filter (applied before all stages) prevents false positives across different value types. The "not in registry" failure message now includes a semantic near-match suggestion (found via normalization + Jaccard + optional judge against original registry entries) when one exists.
...
| Rule | Name | What it checks | OTel spec relationship |
|------|------|----------------|------------------------|
| NDS-002 | Tests Pass | The project's existing test suite still passes after instrumentation. **Coordinator-level check** — not a per-file Tier 2 `ValidationRule`. Enforced by the coordinator's schema checkpoint in `src/coordinator/dispatch.ts`, which runs the test suite at checkpoint intervals and triggers rollback on failure. Registered in `RULE_NAMES` and surfaced in agent directives and PR summaries using the standard rule format. | Project-specific concern — non-destructive instrumentation |
| NDS-003 | Code Preserved | Non-instrumentation lines are unchanged. The agent only added imports, tracer setup, span wrappers, and attributes — it didn't modify business logic. Allowlisted patterns include: OTel imports, tracer setup, span creation/methods, try/catch/finally structural lines, null/undefined guards wrapping `setAttribute`, `if (err instanceof Error)` type-narrowing, `if (span.isRecording())` CDQ-006 guards, re-throws, and return-value captures that feed directly into `setAttribute` (limited to function calls and awaited expressions — not synchronous literals, identifiers, or ternary expressions). **AST stripping (primary mechanism)**: before comparing, all OTel instrumentation nodes are stripped from the instrumented AST using ts-morph — `startActiveSpan` wrappers are unwrapped to their callback body, `span.*` method calls and OTel guard `if` statements are removed, and OTel imports/tracer declarations are dropped. This eliminates the root cause of indentation-induced false positives: after stripping, code that was inside a `startActiveSpan` callback is back at its original indentation depth, so both sides normalize to the same Prettier-canonical form. **Prettier normalization (normalize-both-sides)**: after stripping, both the original source and the stripped instrumented source are normalized through Prettier when Prettier is available. With both sides normalized, the diff only fires on genuine content changes. When Prettier is not available, the check falls back to raw-diff behavior (line trimming handles the remaining indentation differences after stripping) and exposes a warning via `drainNds003Warning()` for coordinator-level reporting. Quote style is inferred from the original source and applied to both normalizations so OTel import boilerplate does not shift quote-style inference. Trailing commas are forced to `none` when not specified by the project Prettier config, preventing Prettier from adding trailing commas when splitting arrays to multi-line. **`multiLine` flag normalization**: The TypeScript parser sets a `multiLine` boolean flag on `ObjectLiteralExpression` and `ArrayLiteralExpression` nodes at parse time — an inline `{ a: 1 }` gets `multiLine=false`, while a multi-line `{\n  a: 1\n}` gets `multiLine=true`. Prettier reads this flag when deciding how to format the node. OTel stripping does not reset these flags, so when the agent expands an inline object literal to multi-line, the flag mismatch causes Prettier to produce different text from each side — firing NDS-003 on a pure formatting change. To prevent this, both sides go through `normalizeMultiLineFlags` (in `nds003-multiline-normalizer.ts`) before Prettier runs. This transform walks both ASTs and rebuilds every `ObjectLiteralExpression` and `ArrayLiteralExpression` with `multiLine=false`, so Prettier sees both sides as inline-intent and formats them identically. **Known limitation**: method chain reformatting (e.g., `a.b().c()` split across lines) uses a different mechanism — leading trivia on dot tokens — and is not addressed by this normalization. It is not currently causing acceptance gate failures; if it becomes a practical problem, it should be addressed in a dedicated PRD. **Return-value capture**: when the agent rewrites `return expr` as `const var = expr; span.setAttribute(...); return var;` for setAttribute, NDS-003 recognizes the structural equivalence — `const var = expr` and `return var` are matched against the original `return expr`. **Multi-line method chain collapse**: when the agent collapses a developer-style multi-line method chain (e.g., `return text\n  .toLowerCase()\n  .replace(...)`) onto a single line, NDS-003 reconciles the two forms as equivalent — the exact same chain content joined is accepted. Content changes (different method names or arguments) still fire. **Multi-line preservation**: the agent must not reduce the line count of any statement or expression (variable declarations, method chains, function call arguments, conditionals). The error message explicitly states this when lines are missing. | Not assessed in PRD #483 audit |
...
| Rule | Name | What it checks | OTel spec relationship |
|------|------|----------------|------------------------|
| SCH-001 | Span Names Match Registry | Span names match the operation names defined in the registry. **Unconditionally blocking** (sparse-registry downgrade removed in [PRD #508](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/508)). **Extension acceptance**: declared span extensions are validated against existing registry operations. Delimiter variants (normalization, e.g., `user_register` ≈ `user.register`) are **blocking** — use the existing registry name. **Exact-match pre-check**: if a declared extension is identical to an existing registry operation (e.g., from function-level fallback aggregating the same span from multiple functions), it is silently skipped — no duplicate error. Without this, normalization would label identical strings "delimiter-variant duplicates." Semantic equivalents (optional LLM judge) produce an **advisory** warning — the agent decides whether to reuse the existing name or keep the distinct name if the operations are hierarchically different (e.g., a CLI dispatcher vs its child operation); blocking caused span name collisions when hierarchically-distinct operations triggered the similarity judge. **Judge namespace pre-filter**: the LLM judge now applies the same namespace pre-filter for span names as for attributes — `taze.check.run` is only compared against `taze.check.*` registry entries, not `taze.io.*` or `taze.cli.*`, preventing false-positive semantic-duplicate advisories on hierarchically distinct operations sharing an app-level namespace prefix. **Naming quality fallback** (no registry span definitions): single-component span names (no dot separator) are always rejected; dotted names must match `/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$/` — both checks are deterministic, no LLM call. **JS/TS only** (uses ts-morph). | Directly aligned — [OTel Trace API — Span](https://opentelemetry.io/docs/specs/otel/trace/api/#span): low cardinality and meaningful operation identification |
| SCH-002 | Attribute Keys Match Registry | Attribute keys match the names defined in the registry (including any OTel semantic conventions the org has imported as a registry dependency). The agent checks the registry only — it does not fall back to OTel semconv from training data. **Unconditionally blocking** (sparse-registry downgrade removed in [PRD #508](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/508)). **Extension acceptance**: declared attribute extensions are validated via normalization (delimiter variants) and optional LLM judge (semantic equivalents); type-compatibility pre-filter prevents false positives across types. Jaccard is intentionally not used for extension acceptance (cannot distinguish legitimate siblings like `http.request.status_code` vs `http.response.status_code`). **"Not in registry" message** includes a semantic near-match suggestion (using Jaccard + judge) when available. **Auto-registration of novel attributes** ([PRD #902](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/902)): novel `setAttribute()` keys not declared in `schemaExtensions` are auto-registered deterministically before SCH-002 validation runs. The pipeline: (1) extract literal string keys from `setAttribute()` calls in the instrumented code (multi-line Prettier-reformatted calls handled); (2) filter out keys already in the registry via exact match, normalization (delimiter variants), and Jaccard similarity; (3) run the LLM judge on surviving candidates to confirm novelty vs semantic duplicate; (4) write confirmed-novel keys to `agent-extensions.yaml` using bare string ID format with defaults `type: string`, `stability: development`, `brief: "Agent-discovered attribute: {id}"` (supplied by `parseExtension`). Agent-declared `schemaExtensions` entries override auto-extracted entries on ID conflict and may carry richer metadata (explicit type, brief, stability) — explicit declaration is the preferred path when the agent knows the attribute semantics. Semantic duplicate candidates dropped by the judge are not registered; SCH-002 fires for them as it does today. **Remaining gap**: dynamic keys (variables, template literals) are not statically extractable and still require explicit agent declaration; dynamic keys are 0% of eval data (PRD #902 Decision Log 2026-06-02/2026-06-03). **JS/TS only** (uses ts-morph). **Known false negative**: when `setAttributes` is called with a pre-built object variable rather than an inline object literal (e.g., `span.setAttributes(attrs)` where `attrs` was constructed earlier), SCH-002 produces no findings — the first argument is not an `ObjectLiteralExpression` so attribute extraction returns empty. Fixing this requires following variable references to their assignment, which needs full TypeScript type resolution. Any PRD adding `setAttributes(variable)` support MUST address this limitation. | Directly aligned — OTel semantic conventions define standard attribute keys |
| SCH-003 | Attribute Values Conform | Attribute values match the types and constraints defined in the registry (enums, integers, strings, booleans). **Literal values**: checked exactly — a string literal for an `int`-typed attribute fails, a number literal for a `boolean`-typed attribute fails, an invalid enum member fails. **Non-literal values** (variables, `.length`, function calls, comparisons): classified by `classifyExpression()` before validation. Classifications: `numeric` (`.length`, `.size`, `parseInt`, `parseFloat`, `Number()`, `Math.*`, arithmetic with `-`, `*`, `/`, `%`); `string` (`String()`, template literals, `.toString()`, `.join()`, `.toFixed()`, `.padStart()`, `.trim()`, etc.); `boolean` (comparisons `===`/`!==`/`>`/`<`/`>=`/`<=`/`&&`/`||`, prefix `!`, `.includes()`, `.some()`, `.every()`, `.has()`, `.startsWith()`, `.endsWith()`, `Boolean()`); `unknown` (unrecognized patterns — silently skipped to avoid false positives). **Auto-fix** (`fixAttributeTypeCoercions`): runs before validation in the instrumentation pipeline. For `string`-typed attributes with a non-literal `numeric` or `boolean` expression, wraps the value in `String()` — e.g., `span.setAttribute("my.label", items.length)` → `span.setAttribute("my.label", String(items.length))`. These auto-fixed cases are intentionally not re-flagged by the validator. **Unfixable mismatches flagged as blocking**: `string` expression for an `int`-typed attribute (cannot safely coerce a string to an integer); `string` or `numeric` expression for a `boolean`-typed attribute (semantics are ambiguous — `Boolean(42)` is always `true`, not a useful signal). **Silently skipped**: `unknown`-classified expressions and all enum-typed attributes with non-literal values. **JS/TS only** (uses ts-morph). | Not assessed in PRD #483 audit |
...
| Rule | Name | What it checks | OTel spec relationship |
|------|------|----------------|------------------------|
| CDQ-001 | Spans Closed | Every span is closed in all code paths — via `span.end()` in a `finally` block or via `startActiveSpan` callback. Unclosed spans leak resources. Additionally, when `process.exit()` is called inside a `startActiveSpan` callback, `span.end()` must be called immediately before each `process.exit()` call to avoid the span being lost during async shutdown. The `fixProcessExitSpanEnd()` auto-fix inserts the required `span.end()` calls before `process.exit()` at the start of each attempt, before validation runs. | Not assessed in PRD #483 audit |
| CDQ-003 | Standard Error Recording | The error recording pattern in catch blocks uses the correct two-call sequence: `span.recordException(error)` followed by `span.setStatus({ code: SpanStatusCode.ERROR })`. Fires when one call is present but the other is missing, or when the calls appear in the wrong order. **Rule file not yet implemented** — registered in `RULE_NAMES` as a placeholder but absent from `tier2Checks`; does not fire until the rule file is added and the rule is registered. | Directly aligned — [OTel Recording Errors](https://opentelemetry.io/docs/specs/semconv/general/recording-errors/): both `recordException` and `setStatus(ERROR)` are required to fully capture an error on a span |
...
| RST-005 | No Double Instrumentation | The agent didn't add spans to functions that already have instrumentation in the original source code. **Never fires in the current system** because spiny-orb only instruments uninstrumented files (early-exit when `originalCounts.size === 0`); preserved for when partial-instrumentation support is added. **Pre-condition**: any PRD adding partial-instrumentation support MUST include a milestone to re-evaluate RST-005 — verify it fires correctly on real partial-instrumentation cases, update the early-exit logic, and confirm test coverage. | Project-specific concern — correctness heuristic |
...
| CDQ-006 | isRecording Guard | Expensive attribute computations (serialization, array operations) are guarded with `span.isRecording()`. External source strings — values fetched from git output, API responses, file contents, or any source whose length is unbounded — are also guarded, even when no computation is involved. When a span is sampled out, the guard skips both the computation and the string fetch. **Exemption**: CDQ-006 does not apply to spans on COV-001 entry points — adding `isRecording()` guards at entry points creates clutter for negligible gain when an entry point span is dropped all child work is dropped too, making the guard moot. | Directly aligned — [OTel Trace API — IsRecording](https://opentelemetry.io/docs/specs/otel/trace/api/#isrecording): "A Span SHOULD avoid doing expensive computations when it's not recording." |
| CDQ-007 | Attribute Data Quality | Two sub-checks on `setAttribute` calls: (1) key is a known PII name (e.g., `email`, `username`) — matched by **exact string equality only**, not substring; `commit.file.name` does not match the PII key `name`; (2) value is a path-like identifier on a non-`file.*` attribute key. The path sub-check exempts OTel `file.*` semantic-convention keys where full paths are spec-correct. **Newly registered in the PRD #483 audit** — rule file existed pre-audit but was not wired into `tier2Checks`. **Import constraint:** Advisory transformations (e.g., using `basename(path)` for path attributes) apply only when the required utility is already imported in the file. The agent will not add new non-OTel imports to comply with this advisory — if the utility is not available, it uses the raw value and notes it as a known limitation. | Mixed: PII sub-check — indirectly consistent; path sub-check — directly aligned after refactor ([file semconv](https://opentelemetry.io/docs/specs/semconv/attributes-registry/file/)) |
| CDQ-009 | Null-Safe Guard | `setAttribute` calls where the attribute value has an unsafe null/undefined guard: either guarded with `!== undefined` rather than the more protective `!= null` (misses null), or using optional chaining (`?.`) without a `?? defaultValue` fallback (silently produces `undefined` when a property is absent). Agent prompt guidance covers both patterns. **Newly registered in the PRD #483 audit.** | Directly aligned — [OTel Common — Attribute](https://opentelemetry.io/docs/specs/otel/common/#attribute) |
