...
- NDS-003 method chain trivia — future PRD after #885 lands ([issue #886](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/886)) — leading dot token trivia causes false positives on method chains near the 80-char boundary after `startActiveSpan` indentation shift; mechanism is distinct from #885's multiLine fix; run-19 confirms it is a practical problem (claude-collector.js partial).
- COV-005: `getCommitData` attribute thinning + 3 missing schema extensions ([issue #887](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/887)) — add `commit_story.git.is_merge` (boolean), `.parent_count` (int), `.command` (string) to schema registry; add prompt guidance requiring `commit_story.commit.message` (with `isRecording()` guard) and `commit_story.commit.timestamp` on `getCommitData`.
