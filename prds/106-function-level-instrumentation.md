...
| 2026-03-14 | Fix re-export detection and add library descriptions to prompt | Evaluation revealed `export { ... }` re-export blocks not detected by extraction (misses most functions in journal-graph.js). Also: LLM over-relies on auto-instrumentation because prompt lacks descriptions of what each library covers. Both fixes needed to meet acceptance gate for journal-graph.js node-level spans. |
