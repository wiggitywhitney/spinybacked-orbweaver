...
- **First step of any rules-related work (PRD milestone zero, or the opening of a rule-changing issue): read `docs/rules-reference.md` in full** — it is the canonical record of every rule's current behavior, scope, and known limitations. Context is cleared between milestones and between issue sessions; re-reading the whole reference document prevents decisions from being inadvertently reverted by a fresh session that lacks context. Reading the whole document leaves less room for error than relying on section-by-section recall.
- **Before opening any PRD that changes validation rules or reconcilers: scan `src/validation/` for conflicts and redundancy.** Read each reconciler's match conditions and compare them against the others — look for overlapping patterns (two reconcilers handling the same structural case), contradictory behavior (one reconciler undoing what another fixed), and rules that have drifted from their documentation in `docs/rules-reference.md`. If a conflict is found, resolve it before proceeding. This step prevents patch accumulation from creating an incoherent rule set.
...
**Before diagnosing any acceptance gate failure, read all five diagnostic dimensions in order.** Even when one dimension appears conclusive (e.g. errorProgression shows a specific rule), the others must still be checked — each answers a different question: (1) history — run `gh run list --workflow=acceptance-gate.yml` (no limit) to see when it last passed; look back as far as needed to find the last green run; (2) actual instrumented code — read `/tmp/spiny-orb-debug-{label}` where `{label}` is the fixture file name passed to `dumpDiagnostics` (e.g., `index.js` → `/tmp/spiny-orb-debug-index.js`), written during every test run; (3) validator error messages — in the `dumpDiagnostics` console output, read `lastError`, which contains full NDS-005 block previews and complete messages for all blocking failures (not just the count summary in `errorProgression`); for oscillation failures, also read `lastErrorByAttempt`; (4) agent notes — read `notes` in the same console output, what the agent reported about its own decisions and what functions were skipped; (5) agent thinking — read `thinkingBlocksByAttempt`, one entry per attempt (oscillation failures need later attempts' reasoning, not just attempt 1). Do NOT write any fix code until all five dimensions have been read. Do NOT diagnose from `errorProgression` summaries alone — they show only error counts, not the specifics needed to identify the root cause.

**Before pushing to CI, confirm the fix passes a local test targeting only the failing fixture files.** The full CI suite takes ~60 minutes; a single-fixture local run takes ~15-20 minutes. Run `vals exec -f .vals.yaml -- bash -c 'for p in /opt/homebrew/bin /usr/local/bin; do [ -x "$p/node" ] && export PATH="$p:$PATH" && break; done && env -u ANTHROPIC_CUSTOM_HEADERS -u ANTHROPIC_BASE_URL npx vitest run --config vitest.acceptance.config.ts --reporter=verbose test/commit-story-v2/acceptance-gate.test.ts -t "fixture-name.js" 2>&1' > /tmp/fixture-run.txt` (substituting the failing fixture name) and confirm it passes before pushing. Do not push to CI when the local run is still failing — you will burn a CI slot and wait another hour. The only exception is an Anthropic API outage (check status.claude.com): if the local run fails with an Internal Server Error, that is a platform failure, not a code failure — rerun after the incident resolves.
