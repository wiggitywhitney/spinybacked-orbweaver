...
| OD-9 | (pending) | | |
| OD-10 | (pending) | | |
| D-PROCESS-1 | Before writing any Go structural-analysis or reassembly code (Milestone E1), enumerate Go's own structural quirks up front and write a test for each one *before* the first implementation pass — do not rely on iterative CodeRabbit review rounds to surface them one at a time. Also diff the Go provider's design against the JavaScript/TypeScript providers' established conventions (e.g. `OTEL_IMPORT_PREFIXES`-style import allowlists, decorator/annotation handling, dedup logic) before writing new code, rather than reimplementing independently and trusting review to catch drift. | PRD #373's Python provider work needed 16 CodeRabbit review rounds on Milestone D1 (extraction/reassembly) alone. The large majority of findings were well-known Python idioms (tabs vs. spaces, multi-line decorators/strings, the `try`/`except ImportError` optional-dependency pattern, `if TYPE_CHECKING` guards, conditionally-defined functions) that could have been enumerated before writing the first line of code rather than discovered reactively. One finding specifically was a direct miss of an existing JS/TS convention (the OTel-import allowlist) that a systematic diff against the sibling providers would have caught immediately. Go has its own analogous quirks needing the same up-front treatment: no indentation significance (braces instead), but build tags (`//go:build`), multi-line raw string literals (backtick-delimited), struct embedding, generics, and `goroutine`/`defer` semantics. | 2026-09-18 |
...
- [ ] **OD-3 (tree-sitter-go):** Run `/research tree-sitter-go` regardless of whether `web-tree-sitter` is already a dependency (added in PRD #373) — that dependency being present only means the *shared binding library* doesn't need reinstalling; it says nothing about the Go grammar's own `.wasm` source, checksum, provenance, or ABI compatibility, which still needs its own verification (mirroring PRD #373 OD-1's research for the Python grammar). Only the *installation* of `web-tree-sitter` itself is conditional: check `package.json` first, and skip re-adding it as a dependency if it's already there. Current recommendation: adopt `web-tree-sitter` (WASM binding) consistent with the pattern established in PRD #373 OD-1, rather than the native `tree-sitter` npm package. **This is provisional, not an unconditional prohibition** — the original rationale (native `tree-sitter` requiring Node 24 for v0.26) is unverified as of 2026-09-17: a quick check of the npm registry (`tree-sitter` latest is `0.25.1`, no `0.26.x` published, no `engines` field) and `node-tree-sitter`'s GitHub `main` branch (same: no `engines` field) found no corroborating evidence for this specific claim, but that is inconclusive rather than a disproof. Treat the binding choice as open until the mandatory `/research tree-sitter-go` step above re-verifies (or replaces) this rationale, and record the actual final binding decision — along with the Go grammar's own `.wasm` source, checksum, provenance, and ABI compatibility — in the Decision Log once that research completes. **Sync/async reconciliation (PRD #373 Decision D-D1-2) — conditional on the binding decision above:** the `LanguageProvider` interface's AST methods (`findFunctions()`, etc.) are synchronous. If research confirms `web-tree-sitter`, its WASM setup is inherently async — `src/languages/python/ast.ts` resolves this with a **top-level `await`** at module load (`await Parser.init()`, `await Language.load(...)`), which ESM guarantees resolves before any importer sees the module's exports; reuse that same pattern for `src/languages/go/ast.ts`. If research instead selects a native `tree-sitter` binding, follow whatever initialization model that binding actually requires (native bindings are typically synchronous, so the top-level-`await` pattern may not even apply) — do not assume the Python pattern carries over unmodified.
...
- [ ] **(Updated per Decision D-PROCESS-1)** Before writing any Go structural-analysis or reassembly code, enumerate Go's own structural quirks that parallel what tripped up PRD #373's Python work — build tags (`//go:build`), multi-line raw string literals (backtick-delimited), struct embedding, generics, `goroutine`/`defer` semantics — and write a test for each *before* the first implementation pass. Separately, systematically diff the planned Go provider design against the JS/TS providers' established conventions (import allowlists like `OTEL_IMPORT_PREFIXES`, decorator/annotation handling, dedup logic) rather than reimplementing independently.
...
**Cross-reference — PRD #373 Decision D-D3-1 (2026-09-19):** `classifyFunction()` should stay a permanent `'unknown'` stub for Go too, matching the convention already established by all three other providers (JavaScript, TypeScript, Python). `FunctionClassification` needs call-pattern context (route-registration decorator/call shape) that bare `FunctionInfo` doesn't carry, so real entry-point detection for `http.HandleFunc`/`http.Handler`/gRPC/Gin/Echo/Fiber belongs in Milestone E3's `cov001.ts`, which re-walks the Go AST directly — not in `classifyFunction()` below. Don't rediscover this split mid-implementation.
...
- [ ] For each shared-concept rule, implement Go-specific version:
  ...
  - **Portable rules — same implementation, `applicableTo('go') = true`:** `sch003` — attribute value type conformance against the registry schema; no language-specific AST parsing needed. Note: `sch004` was deleted (PRD #508). `sch001` and `sch002` are NOT portable (PRD #508 D-4, D-5): both use ts-morph internally and are restricted to JS/TS; the Go provider must implement its own `sch001.ts` (span name extraction) and `sch002.ts` (attribute key extraction) with Go-specific AST parsing (no ts-morph). The shared `checkSemanticDuplicate` helper in `src/languages/javascript/rules/semantic-dedup.ts` has no ts-morph dependency and can be imported by Go implementations for the normalization/Jaccard/judge pipeline. `cdq008` was deleted (PRD #505).
  ...
  - **Cross-reference — PRD #373 Decision D-D3-3 (2026-09-23):** this list omits `rst006`, `nds007`, `cdq005`, `cdq007`, `cdq011`, `api004`, and `cov005` — Python's equivalent evaluation (Decision D-D3-3) found all seven of these apply to Python and need a language-specific implementation there, none excluded for a Python-specific reason. Before starting this milestone, re-check whether the same holds for Go (e.g. `cdq011`'s canonical-tracer-name check and `api004`'s forbidden-SDK-import check are pure text/regex checks with no ts-morph dependency in JS, and ported to Python with only a syntax-pattern change) rather than assuming this list's omission of them was itself a considered exclusion — it may simply predate that evaluation.
