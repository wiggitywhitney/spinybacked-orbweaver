...
- [ ] **Milestone 1: Per-file extension writing in dispatch loop** — Move `writeSchemaExtensions()` call into the `dispatchFiles()` for-loop, executed after each successful file (after `instrumentFn` returns with `status: 'success'`). Extensions from the current file are written to `agent-extensions.yaml` before the next file's `resolveSchema()` call. The existing `collectSchemaExtensions` + `writeSchemaExtensions` functions may need adjustment to support incremental writing (appending to existing extensions rather than overwriting). Verify: (a) file B's resolved schema includes extensions from file A, (b) `weaver registry check` passes after each incremental write, (c) extensions accumulate correctly across files.
...
1. **Incremental vs overwrite for `agent-extensions.yaml`**: The current `writeSchemaExtensions` overwrites the file each call. Per-file writing needs to append (accumulate extensions from files 1..N). Options: (a) read-modify-write the YAML file each time, (b) maintain an in-memory accumulator and overwrite with the full set each time, (c) write one file per source file (`agent-extensions-{filename}.yaml`) — Weaver resolves the whole directory so multiple files work. Option (b) is simplest and matches the current function signature. Option (c) makes per-file revert trivial (delete the file) but creates more filesystem clutter.
2. **Schema revert granularity**: The spec says snapshot "schema state." For `agent-extensions.yaml` specifically, is it sufficient to snapshot just that file, or does revert need to cover any file the agent might have modified in the registry directory? Since `writeSchemaExtensions` only writes `agent-extensions.yaml`, snapshotting that single file should be sufficient.
