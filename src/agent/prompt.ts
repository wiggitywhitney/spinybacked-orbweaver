...
- **CDQ-007**: Do NOT set unbounded attributes (full object spreads, unsized arrays), PII fields, or undefined values. PII attribute keys to avoid (exact matches only, not substrings): \`author\`, \`committer\`, \`username\`, \`email\`, \`password\`, \`ssn\`, \`name\`, \`user\`. Do not conflate these with attribute keys that CONTAIN one of these words — \`commit.file.name\` is not a PII attribute. For raw filesystem path values (variables named \`filePath\`, \`outputDir\`, etc.), sanitize in this order: (1) if \`basename\` is already imported from \`node:path\`, call it matching how it was imported — \`basename(...)\` for a named import (\`import { basename } from 'node:path'\`), or \`path.basename(...)\` only if the file imports the whole module as \`path\`; (2) otherwise, use inline string manipulation — \`filePath.split(/[\\\\/]/).filter(Boolean).pop() ?? ''\` strips the directory portion with zero imports (the character class matches both forward-slash and backslash separators, covering POSIX and Windows paths; filtering empty segments before \`.pop()\` handles paths with a trailing separator, which would otherwise yield an empty string). Import constraint: do NOT add any new non-OTel import — including \`basename\` — to satisfy this advisory; steps 1-2 above never require one. Only fall back to the raw value and note it as a known limitation in \`notes\` when the path-like value is not a plain variable reference (e.g., it's a property access or function call) — CDQ-007's path check only fires on bare identifier values, so neither sanitization step applies outside that case.
