...
These follow the Part 8 checklist from the research doc. All OD-1 through OD-7 design decisions are resolved (see Decision Log above). Milestone D1 is partially implemented — directory scaffolding, `ast.ts`'s structural-analysis functions, function-level extraction/reassembly, and `checkSyntax()`/`formatCode()`/`lintCheck()` are done (see the checked items below); the remaining unchecked items (file discovery config, `otelSemconvPackage`/`packageManager`/`installCommand()`, provider registration in `registry.ts`, and the config schema enum extension) are still to be built. Note for whoever picks up the remaining wiring next: reuse `parsePython()` exported from `src/languages/python/ast.ts` for any tree-sitter parsing needs rather than initializing a separate `Parser`/`Language` instance — see Decision D-D1-2 and the `extraction.ts`/`reassembly.ts` modules for the established pattern. Also note the tree.delete() gotcha found during this milestone: read every field off a tree-sitter `Node` (e.g. `.startPosition`, `.endPosition`, `.text`) into a plain value *before* calling `tree.delete()` on its tree — reading a `Node`'s fields after the tree is deleted silently returns stale/zeroed data rather than throwing.

**Per Decision D-D1-3 (2026-09-18):** Milestones D1 and D2 are implemented and PR'd together, not separately. `PythonProvider` cannot compile or register without real `getSystemPromptSections()`/`getInstrumentationExamples()` content, and that content is D2's `prompt.ts`. Whoever completes D1's remaining checklist items should build D2's `prompt.ts` in the same pass and check off both milestones' items when done.
...
- [ ] Register `PythonProvider` in `src/languages/registry.ts` for `.py`. Whichever fix-loop code ends up calling `reassemblePythonFunctions()` (currently called from nowhere — `extraction.ts`/`reassembly.ts` are not yet wired into the fix loop) must pass `extracted`/`results` as index-aligned, parallel arrays, not build `results` by looking up a `FunctionResult` by matching function name — `reassemblePythonFunctions()`'s own pairing is already index-based specifically because two methods in different classes can share a name (see the `reassembleFunctions()` D1 checklist entry above). A CodeRabbit finding raised this for a hypothetical future caller; there's no such caller yet, so there was nothing to fix in this milestone — but the next implementer to wire Python into the fix loop needs this context. When creating `src/languages/python/index.ts`'s `PythonProvider` class, note that `checkSyntax()` in `validation.ts` is synchronous (returns `CheckResult` directly, not a `Promise`) and must be wrapped in `Promise.resolve(...)` at the class method boundary — matching `javascript/index.ts` and `typescript/index.ts`'s own `checkSyntax()` methods exactly. `formatCode()` and `lintCheck()` already return `Promise`s and need no wrapping.
...
**Per Decision D-D1-3 (2026-09-18): implemented and PR'd together with Milestone D1, not as a separate follow-up.** `PythonProvider`'s required `getSystemPromptSections()`/`getInstrumentationExamples()` methods need this milestone's `prompt.ts` to exist before D1's registration and typecheck/test success criteria can be met.
