...
- P3 fix loop acceptance gate intermittently returns \`partial\` instead of \`success\` on \`user-routes.js\` ([issue #759](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/759)) — first step: add \`dumpDiagnostics\` to the test so future failures produce a diagnosable artifact. — confirmed root cause: `caffeinate -s` + `vals exec -i` strips HOME, Weaver can't find `~/.weaver/vdir_cache`, hangs on OTel semconv download. Two-line fix in `checkWeaverSchema`.
