...
Phase 4 introduces new Tier 2 checks (COV and RST) enabled by multi-file context, and applies RST-005 (already-instrumented detection) at the coordinator level.

| Rule | Name | Tier | Blocking? | Description | Automation |
|------|------|------|-----------|-------------|------------|
| RST-005 | No Re-Instrumentation of Already-Instrumented Code | 2 | Yes (Important) | AST: detect functions that already contain `startActiveSpan`, `startSpan`, or `tracer.` calls in the pre-agent source; flag if the agent adds additional tracer calls. At the coordinator level, this means detecting already-instrumented files and returning `status: "skipped"`. | Automatable |
| COV-001 | Entry Points Have Spans | 2 | Yes (Critical) | AST: detect framework from `package.json` dependencies, then find entry point operations using framework-specific patterns (Express: `app.get/post/put/delete()`, `router.*()` callbacks; Fastify: route handlers; raw http: `createServer()` callback; exported async functions from service modules); verify each has a span. | Automatable |
...
| COV-003 | Failable Operations Have Error Visibility | 2 | Yes (Important) | AST: for each COV-001/COV-002 site plus any operation in a pre-existing try/catch block, verify the enclosing span has any error recording call (`recordException`, `setStatus`, or `span.setAttribute` with error-related keys). | Automatable |
| COV-004 | Long-Running / Async Operations Have Spans | 2 | Advisory (Normal) | AST: find `async` functions, functions containing `await` expressions, and calls to known I/O libraries; verify each has a span. | Automatable |
...
| COV-006 | Auto-Instrumentation Preferred Over Manual Spans | 2 | Yes (Important) | Check whether manual spans target operations covered by known auto-instrumentation libraries; flag manual spans on those operations. | Automatable |
...
| RST-003 | No Duplicate Spans on Thin Wrappers | 2 | Yes (Important) | AST: flag spans on functions whose body is a single return statement calling another function (possibly with argument transformation). | Automatable |
| RST-004 | No Spans on Internal Implementation Details | 2 | Advisory (Normal) | AST: flag spans on unexported functions and private class methods. **Exception**: unexported functions that perform I/O or external calls are exempt. | Automatable |

**Blocking/advisory classification rationale**: COV-001, COV-002, COV-003, COV-006, RST-003, RST-005 are blocking because they represent correctness issues (missing spans on critical paths, manual spans where auto-instrumentation exists, re-instrumentation, thin wrapper duplication). COV-004, COV-005, RST-001, RST-002, RST-004 are advisory because they represent quality improvements worth flagging for human review but not worth reverting a file over.
...
| Periodic Schema Checkpoints | Basic interval only | 512–516 | NOT the full Weaver diff integration (Phase 5). Basic `weaver registry check` every N files, stop-on-failure default. |
...
**`schemaDiff`, `schemaHashStart`, `schemaHashEnd`, and `endOfRunValidation`** are defined in `RunResult` but populated by Phase 5 (schema integration). Phase 4 sets them to `undefined`. This keeps the type stable across phases — Phase 5 extends the coordinator to fill these fields without changing the return type.
...
- [ ] **Milestone 2: Already-instrumented detection** — Implement file-level detection of existing OTel instrumentation: check for `@opentelemetry/api` imports, `tracer.startActiveSpan`/`startSpan` calls, and imported tracer factory patterns. Files with existing instrumentation return `FileResult` with `status: "skipped"`. Verify: (a) inline tracer patterns detected, (b) imported tracer factory patterns detected, (c) skipped files appear in results with correct status → RST-005.
...
- [ ] **Milestone 5: SDK init file writing and dependency installation** — After all files processed: aggregate `librariesNeeded` from all results, write SDK init file (find `NodeSDK` instrumentations array via ts-morph, append new entries with imports), run bulk `npm install` respecting `dependencyStrategy`. Verify: (a) SDK init file contains all discovered libraries, (b) `@opentelemetry/api` is always peerDependency, (c) `dependencyStrategy: dependencies` uses `npm install --save`, (d) `dependencyStrategy: peerDependencies` uses `npm install --save-peer` and adds `peerDependenciesMeta`, (e) individual package install failures are degraded (not fatal) and reported in `libraryInstallFailures`.
...
- Periodic schema checkpoint with diff output → Phase 5 (Phase 4 runs basic `weaver registry check` at intervals)
...
1. **Periodic schema checkpoint scope**: The phasing document says Phase 4 gets "basic interval only" for periodic schema checkpoints (running `weaver registry check` every N files). The full checkpoint logic (with `weaver registry diff`, blast radius reporting, and `onSchemaCheckpoint` callback behavior) is Phase 5. Should Phase 4 implement the basic `weaver registry check` interval, or defer all periodic checkpoint logic to Phase 5? The acceptance gate doesn't explicitly require it, but the spec lists it as a coordinator responsibility.

2. **Already-instrumented detection granularity**: The spec describes file-level detection (skip entire files with existing OTel code). The evaluation found that inline tracer patterns were correctly detected but imported tracer factory patterns were not (F15, F25). Should the coordinator do a quick file-level check (presence of `@opentelemetry/api` imports) and delegate more nuanced detection to Phase 1's agent (which already handles this), or implement the full pattern matching at the coordinator level?

3. **SDK init file fallback**: The spec describes a fallback when the SDK init file doesn't match a recognized pattern — write a separate `orb-instrumentations.js` file with instructions. Should this be implemented in Phase 4, or deferred as DX polish to Phase 7? The acceptance gate requires "SDK init file is correctly updated," which implies the recognized pattern works, but the fallback path should at least produce a clear warning rather than silently fail.
