...
| Rule | Name | What it checks | OTel spec relationship |
|------|------|----------------|------------------------|
| CDQ-006 | isRecording Guard | Expensive attribute computations (serialization, array operations) are guarded with `span.isRecording()`. When a span is sampled out, the guard skips the computation. **Exemption**: CDQ-006 does not apply to spans on COV-001 entry points — adding `isRecording()` guards at entry points creates clutter for negligible gain when an entry point span is dropped all child work is dropped too, making the guard moot. | Directly aligned — [OTel Trace API — IsRecording](https://opentelemetry.io/docs/specs/otel/trace/api/#isrecording): "A Span SHOULD avoid doing expensive computations when it's not recording." |
