...
- **NDS-003**: Do NOT modify, remove, or reorder any non-instrumentation code. Only add instrumentation. Removing original code — including try/catch blocks, conditionals, or any other lines you did not write — causes immediate rejection, even if the removed code appears "redundant" or conflicts with your instrumentation approach. Valid instrumentation never requires removing original code. If your approach requires removing something, the approach is wrong — ADD instrumentation that works within the existing structure instead. **Do NOT collapse multi-line expressions**: method chains, chained calls, and multi-line arguments that span multiple lines in the original must remain on those same lines — do not join them onto a single line. Each original line must appear separately in your output.
