...
- Static validator: regex literal syntax errors ([issue #957](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/957)) — regex literal patterns cause NDS-001 tsc failures; deterministic AST check catches the pattern before tsc runs. Juggling order: 1.
- CDQ-006 isRecording guard: prompt instruction and AST validator ([issue #956](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/956)) — prompt comprehension failure causes inconsistent guard application; prompt is the primary fix, AST validator is a backstop. Juggling order: 2.
