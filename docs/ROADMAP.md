...
- Schema extension reliability — structural enforcement ([PRD #901](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/901)) — add retry prompt carve-out and class-based self-verification checklist; COV-005 blocking deferred (registryDefinitions never populated — see PRD #901 Decision Log); behavioral fix for agents failing to register new schema attributes.
