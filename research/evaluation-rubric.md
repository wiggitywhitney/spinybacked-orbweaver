...
| **Sub-classifications** | **NDS-005a (Structural breakage)**: The agent restructured, merged, removed, or reordered pre-existing error handling blocks, altering error propagation semantics. Evaluated via AST diff of error handling structures. **NDS-005b (Expected-condition recording)**: The agent preserved the error handling structure but added `recordException()` + `setStatus(ERROR)` in catch blocks that handle expected conditions (validation failures, missing optional resources, graceful fallbacks). Identified by checking whether the original catch block was a "silent catch" — returns a default/fallback value without rethrowing. Both sub-classifications carry Important impact; NDS-005a is more severe but NDS-005b is more common. |
