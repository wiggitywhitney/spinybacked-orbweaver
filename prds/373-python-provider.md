...
| OD-1 | tree-sitter-python via web-tree-sitter, with the grammar `.wasm` sourced from tree-sitter-python's GitHub Releases (not the npm package) | web-tree-sitter has no Node engine restriction, confirming it avoids native tree-sitter's Node 24 requirement; but the `tree-sitter-python` npm package ships a native Node addon (`node-gyp-build` install script), not a usable `.wasm` — the GitHub release for the same version publishes `tree-sitter-python.wasm` directly, avoiding both the native-build problem and an extra local CLI build step. Full findings: `docs/research/web-tree-sitter-python-parser.md`. | 2026-09-17 |
