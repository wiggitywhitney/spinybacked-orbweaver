...
2. **File content heuristic** (fallback) — If `package.json` is missing or unreadable, scan the file content for `import` declarations or `export` keywords. Presence indicates ESM; absence indicates CJS. This is the least reliable signal and exists only as a fallback when the primary signal is unavailable.
