...
**Practical implication for PRD #980 M2:** Waiting longer or using a workaround (raw-query bypass, API-based tag configuration lookup) will not surface this metric in the UI. The metric needs a fresh data point. Live span inspection (2026-07-06/07, this repo's session) rules out the post-commit hook as the blocker — both commit-story-v2 and spinybacked-orbweaver hooks already conditionally add `--import examples/instrumentation.js` when present, confirmed active in production via `process.command_args` on real spans. Instead, LLM-call spans currently carry `gen_ai.request.*` attributes but no `gen_ai.usage.output_tokens` / `gen_ai.usage.input_tokens` attribute — the likely actual data source for this metric appears absent from current instrumentation. commit-story-v2's Weaver registry already declares both attributes under the gen_ai/inference-client attribute group at `requirement_level: recommended` — no schema change is needed to restore live data for this fix. A fresh data point requires the missing `span.setAttribute()` calls on commit-story-v2's spans (tracked in PRD #980's M1.5). PRD #1024 only affects enforcement of already-required attributes; it does not govern whether `gen_ai.usage.*` exists at all. Separately, PRD #980's M1.7 does add a schema change here — promoting these attributes from `recommended` to `required` — needed not to fix this immediate data gap but to make the attribute's presence durable across future spiny-orb instrumentation runs, since `recommended`-level declarations aren't a forcing function for the agent to add them. Once a new data point lands, it should reappear in Summary search / Explorer autocomplete within the same window normal newly-created metrics do (not separately delayed).
