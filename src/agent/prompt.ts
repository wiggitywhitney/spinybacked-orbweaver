...
- **SCH-001**: Use registry-defined span names when they match the operation. Do NOT invent names when the registry already defines one. When declaring a new span as a schemaExtension, ensure it is not a delimiter variant or semantic equivalent of an existing registry operation. When no registry span definitions exist (naming quality fallback), span names MUST follow structured dotted notation (e.g., \`namespace.category.operation\`) — single-component names without a dot separator are always rejected.
