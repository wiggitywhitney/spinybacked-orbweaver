...
**Step 0**: Read `docs/demo/traces-metrics-setup.md` in full. This document contains the confirmed metric names, dimension names, and Collector config — all context required for this milestone. Do not skip this step.

**Step 1**: Run `/research Datadog span_metrics metric-to-trace linking: is it possible to click a span_metrics data point in Metrics Explorer or a Datadog dashboard and navigate to the contributing traces? What is this feature called in Datadog? What configuration is required? Does it work with span_metrics generated by the OTel Collector span_metrics connector (as opposed to Datadog APM native metrics)?`
...
**Step 2**: Record findings in the Decision Log with the row title "metric-to-trace linking". The row must answer: (a) is it supported; (b) what it is called in Datadog; (c) what configuration is required; (d) whether span_metrics from the otelcol span_metrics connector support it or only Datadog-native APM metrics do.
