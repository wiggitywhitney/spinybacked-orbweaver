...
**Step 3:** Write the rebuilt diagram directly into the QMD as a single complete slide (added alongside the old sequence, per the diagram-rebuild process — do not overwrite yet). Render with `quarto render talk/slides-llmday/index.qmd` and tell Whitney to view it in the browser. Wait for her explicit approval on the shape there before building out the rest of the progressive-build sequence — per her instruction to "build everything, cull later," all four mechanisms go in now even though the 20-minute slot may require cutting some in a later pass. Whitney does not approve diagram shape from a Mermaid code block in conversation (2026-07-27 decision) — approval always happens on rendered, browser-viewed output.
...
**Step 4:** Write the updated diagram directly into the QMD as a single complete slide (added alongside the old sequence, per the diagram-rebuild process — do not overwrite yet). Render with `quarto render talk/slides-llmday/index.qmd` and tell Whitney to view it in the browser. Wait for her explicit approval on the shape there before building out the rest of the progressive-build sequence. Whitney does not approve diagram shape from a Mermaid code block in conversation (2026-07-27 decision) — approval always happens on rendered, browser-viewed output.
...
- **Diagram-rebuild process clarified for M3 and M4 (2026-07-27).** Both milestones rebuild a diagram that's actually a progressive-build "unfolding" (one element added per slide, ending at a slide showing the complete diagram) — not a single static image. The rebuild process for each:
  ...
  2. **Modify the duplicate to the target architecture, write it into the QMD as a single complete slide, and render it.** Whitney reviews and approves the shape from the rendered, browser-viewed slide (already specified in M3 Step 3 / M4 Step 4) — never from a Mermaid code block pasted into conversation. A Mermaid block is not human-readable to her; it is not a valid approval artifact for this or any diagram-rebuild milestone (see 2026-07-28 decision below).
...
- **Mermaid-in-chat is not a valid approval artifact (2026-07-28).** A Mermaid code block pasted into conversation is not human-readable to Whitney — she cannot evaluate diagram shape from it. This removes the "present as a Mermaid code block in conversation, wait for approval on shape" step that previously appeared in the 2026-07-27 diagram-rebuild process (step 2 above) and in M3 Step 3 / M4 Step 4. The corrected process: write the diagram directly into the QMD as a single complete slide, run `quarto render`, and have Whitney approve from the rendered output viewed in the browser — the same rendered-output approval mechanism already used for every other slide in this PRD. This applies to every diagram-rebuild milestone (M3, M4, and any future ones), not just the milestone in progress when the correction was made.
- **PRD abandoned mid-M3 (2026-08-18).** Whitney has no near-term need for this deck and doesn't have time right now to keep refining it. Working through M1–M3 surfaced a pattern: most of the remaining milestones (M3's topological-ordering/test-gate/live-check diagram additions, M4's per-rule-ID auto-fix distinction) are accuracy nuances that matter to someone already deep in the codebase, not to a first-time audience — the kind of detail that doesn't change what the talk teaches a stranger. M4's own design decision (rule IDs banished to speaker notes, generic label on the slide) was itself a signal that the distinction didn't belong on-screen. The exception is M5 (live Datadog click-through), which is a different category of value — proof the system works, not an accuracy nuance — and is worth doing on its own if this deck is presented again, but not worth reopening this PRD for in isolation. M3's in-progress draft diagram (topological ordering, baseline test gate, Weaver live-check/rollback) was removed from `index.qmd`, reverting the deck to its last Whitney-approved state (PRD #847, M4). M1 and M2 remain shipped and committed — that work is real and stays. M3–M6 are not implemented; the tracking issue closes as abandoned, not completed.
- **PRD archived to `prds/done/` despite not completing (2026-08-18).** The prior entry said this PRD would stay in `prds/` since it didn't ship — but an inactive, abandoned PRD left in the live folder risks a future session picking it back up as open work. `prds/done/` is used here for "no longer live," not strictly "shipped": Whitney explicitly asked to move it out of the active folder so it doesn't get mistaken for in-progress work.
