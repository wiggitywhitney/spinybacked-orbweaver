...
| **Terminal** | Network failure | Anthropic API is unreachable (DNS, timeout, connection refused) | While network issues can be transient, the fix loop is not the right retry layer — it operates at the file level, not the request level. Network retries belong in the HTTP client (with exponential backoff). If the network is down for all 3 attempts, the file fails. |
