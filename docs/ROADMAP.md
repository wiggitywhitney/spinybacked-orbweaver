...
- Fix-loop labels a retry give-up as SUCCESS instead of FAILURE, hiding coverage regressions ([issue #1062](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1062)) — an attempt that reverts to the unmodified original after a blocking validator failure trivially passes NDS-003 and reports SUCCESS with zero spans added, even when the file's own retained advisory findings (e.g. COV-004) contradict that verdict; this can silently mask coverage regressions in any future eval run.
- Run baseline test check before topological sort in coordinator ([issue #1047](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1047)) — Step 2b (dep graph + topo sort) currently runs before Step 2c (baseline test check), wasting work when the baseline check aborts the run; reorder so the cheaper, more decisive gate runs first.
