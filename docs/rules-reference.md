...
| Rule | Name | What it checks | OTel spec relationship |
|------|------|----------------|------------------------|
| NDS-003 | Code Preserved | Non-instrumentation lines are unchanged. The agent only added imports, tracer setup, span wrappers, and attributes — it didn't modify business logic. Allowlisted patterns include: OTel imports, tracer setup, span creation/methods, try/catch/finally structural lines, null/undefined guards wrapping `setAttribute`, `if (err instanceof Error)` type-narrowing, `if (span.isRecording())` CDQ-006 guards, re-throws, and return-value captures that feed directly into `setAttribute` (limited to function calls and awaited expressions — not synchronous literals, identifiers, or ternary expressions). **Multi-line method chain collapse**: when the agent collapses a developer-style multi-line method chain (e.g., `return text\n  .toLowerCase()\n  .replace(...)`) onto a single line, NDS-003 now reconciles the two forms as equivalent — the exact same chain content joined without whitespace is accepted. Content changes (different method names or arguments) still fire. **Multi-line setAttribute arguments**: when the agent formats `span.setAttribute()` across three lines (opening call / key string / value expression / closing `);`), NDS-003 now reconciles the key and value lines as instrumentation — they are only removed from the "unexplained additions" list when the full 4-line pattern is confirmed. **Multi-line preservation**: the agent must not reduce the line count of any statement or expression — variable declarations split across lines, method chains, function call arguments, conditional expressions, or any other multi-line construct must all be preserved with the same line count as the original. The error message now explicitly states this when lines are missing or unexpectedly added due to collapsing. **Prettier normalization (normalize-both-sides)**: before diffing, both the original source and the instrumented source are normalized through Prettier when Prettier is available. Normalizing both sides at their respective indentation depths eliminates false positives caused by Prettier formatting code differently when `startActiveSpan` adds indentation: a line at the original indent that Prettier preserved as one line may expand to multiple lines at the deeper indent inside the span callback. With both sides normalized, Prettier's formatting decisions are applied consistently to each, and the diff only fires on genuine content changes. When Prettier is not available, the check falls back to raw-diff behavior and exposes a warning via `drainNds003Warning()` for coordinator-level reporting. Quote style is inferred from the original source and applied to both normalizations so OTel import boilerplate (which uses double-quoted strings) does not shift quote inference for the instrumented side away from the project's style. Trailing commas are forced to `none` when not specified by the project Prettier config, preventing Prettier from adding trailing commas when it splits an array like `[id]` to multi-line — `[id,]` vs `[id]` would otherwise break `reconcileAgentSplitLines`' token comparison even though the code is semantically identical. Residual mismatches where a line is below Prettier's printWidth at the original indent but above it inside the span callback (the 79-char boundary case) are handled by `reconcileAgentSplitLines` and `reconcileIndentReformat`. | Not assessed in PRD #483 audit |
