...
| 2026-06-04 | M6 agent prompt changes reviewed with `/write-prompt`; four issues fixed inline, none deferred | Initial prompt edits had: (1) stale "SCH-002 rejection" language on the registered-attribute-keys header that conflicted with the new auto-registration behavior; (2) line 159 framed auto-registration as a primary path rather than a reliability backstop; (3) pre-submission SCH check had a false opt-out conditional ("when I know their type, brief, and stability") that would increase skipped declarations; (4) SCH-002 Scoring Checklist bullet was inconsistent with the rest of the prompt (no mention of auto-registration). All four fixed inline per project rule (never defer to GitHub issues). M6 scope expanded to include the Scoring Checklist bullet fix. |
...
- [x] **M6 — Update docs/rules-reference.md and agent prompt**: Per rules-related work conventions, read `docs/rules-reference.md` in full before making changes. Update the SCH-002 entry to document: (1) novel attributes not declared in `schemaExtensions` are now auto-registered deterministically using the OQ-4 default format (bare ID written with `type: string`, `stability: development`, `brief: "Agent-discovered attribute: {id}"` defaults from `parseExtension`); (2) agent-declared extensions override auto-extracted entries on ID conflict and may provide richer metadata (type, brief, stability); (3) truly dynamic keys (variables, template literals) are not extracted and still rely on explicit agent declaration — this is the only remaining gap (dynamic keys are 0% of eval data; see Decision Log 2026-06-02/2026-06-03). Also add a "SCH-002 auto-registration (2026-06-04)" section under post-audit structural changes. Update `src/agent/prompt.ts` at four touch-points: (a) the registered-attribute-keys header (remove stale "rejection" language); (b) the paragraph after "Report new schema entries" (frame auto-registration as backstop, not primary path; note dynamic keys require explicit declaration); (c) the SCH-002 Scoring Checklist bullet (add auto-registration behavior for consistency with the rest of the prompt); (d) the pre-submission SCH check (remove false opt-out conditional; require declaration of every key with type/brief/stability; call out dynamic keys explicitly). Run `/write-docs` to validate documentation changes; run `/write-prompt` on both the PRD and the agent prompt changes. (Updated per Decision Log 2026-06-04: four inline fixes identified by write-prompt review; SCH-002 Scoring Checklist bullet added to scope.)
