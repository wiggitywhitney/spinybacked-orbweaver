...
### No New External Dependencies Beyond `yaml`

Phase 5 uses capabilities already installed from prior phases (`@anthropic-ai/sdk`, `ts-morph`, `zod`) plus Node.js built-ins (`node:fs`, `node:child_process`). The only new dependency is `yaml` v2.8.2 for schema extension YAML generation.
...
- [ ] **Milestone 1: Schema extension YAML generation** — Implement the agent's ability to create new YAML entries for custom attributes in the registry. The agent follows existing schema patterns: namespace prefix is mandatory, structural patterns match existing groups, entries use the project's naming convention. New extension entries are written to the registry directory. Verify: (a) agent creates a valid YAML entry for a custom attribute not in the existing schema, (b) the entry uses the project namespace prefix, (c) `weaver registry check` passes on the extended schema → SCH-001, SCH-002, SCH-003.
...
- [ ] **Milestone 4: Periodic schema checkpoints** — Implement `weaver registry check` every `schemaCheckpointInterval` files (default: 5). Wire the `onSchemaCheckpoint` callback. On checkpoint failure, stop processing new files by default. Files committed before the failing checkpoint are valid. The coordinator reports: which rule failed, which file triggered it, and the blast radius (files processed since last successful checkpoint). Verify: (a) checkpoints run at correct intervals, (b) `onSchemaCheckpoint` fires with `filesProcessed` and `passed`, (c) returning `false`/`void` stops processing, (d) returning `true` continues despite failure, (e) failure report includes rule, file, and blast radius.
...
- [ ] **Milestone 6: End-of-run Weaver live-check** — Implement the end-of-run validation workflow: start `weaver registry live-check -r <path>` (listens on `:4317` gRPC and `:4320` HTTP `/stop`), run test suite with `OTEL_EXPORTER_OTLP_ENDPOINT=localhost:4317`, stop Weaver via `curl localhost:4320/stop`, parse compliance report. Store report in `RunResult.endOfRunValidation`. Handle graceful degradation: if tests don't exist, warn and skip live-check; if ports are in use, report clear error. Verify: (a) Weaver starts and listens, (b) test suite runs with endpoint override, (c) compliance report is captured, (d) missing test suite produces warning (not error), (e) port conflict produces clear error.
...
- **External**: Node.js >=24.0.0, Weaver CLI >=0.21.2 (for `registry check`, `registry resolve`, `registry diff`, `registry live-check`), Anthropic API key, `yaml` npm package (v2.8.2, new dependency), a test JavaScript project with a Weaver telemetry registry for acceptance testing, `testCommand` configured (for end-of-run live-check).
...
1. **Schema extension file placement**: When the agent creates new schema entries, should they be appended to existing YAML files (e.g., `attributes.yaml`) or written to new files (e.g., `agent-extensions.yaml`)? The spec says the agent can "extend schema (create spans, attributes, groups)" but doesn't specify file-level organization. Writing to separate files makes agent contributions identifiable and reviewable. Appending to existing files keeps the registry structure flat. The implementing AI may need to decide based on how Weaver resolves multi-file registries.

2. **Live-check port conflict recovery**: The spec notes ports `:4317` and `:4320` must be available for Weaver live-check. Init verifies port availability. But between init and end-of-run validation, a local OTel Collector or another process could start. Should the coordinator re-check ports before starting live-check, or fail with a clear error and let the user resolve it? The spec suggests "recovery strategies (process detection, reuse, force-clean flags) are implementation decisions."

3. **Checkpoint diff vs. check semantics**: The spec says periodic checkpoints run both `weaver registry check` (validation) and `weaver registry diff` (change tracking). If `check` passes but `diff` shows non-`added` changes, is that a checkpoint failure? The coordinator's "reject any change type other than `added`" enforcement suggests yes — but this is enforcement of the "extend only" constraint, separate from Weaver's own validation. The implementing AI should clarify whether diff-based enforcement is a blocking checkpoint failure or an advisory warning.
