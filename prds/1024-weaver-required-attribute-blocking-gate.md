...
- [ ] **M6 — Verify required-attribute enforcement against commit-story-v2's promoted token-usage attributes**:
  **Step 0:** Confirm PRD #980's M1.7 (Updated per the 2026-07-08 "M1.7 scope split" decision below) has landed on commit-story-v2's `main` — `telemetry/registry/attributes.yaml` should declare `gen_ai.usage.output_tokens`/`gen_ai.usage.input_tokens` (and possibly `commit_story.ai.section_type`) at `requirement_level: required`. This entry must exist before this milestone begins; PRD #980's M1.7 gates this milestone. If M1.7 has not landed yet, this milestone is blocked — do not attempt to promote the attributes yourself; that promotion belongs to PRD #980.
  Run a fresh spiny-orb instrumentation run against commit-story-v2 (or a comparable fixture reproducing the same span sites) on a new branch and confirm the agent includes the now-required attributes without a manual code edit, and that COV-005 (with M1's blocking gate active) fails the run if it doesn't. Record the result in this PRD's Decision Log.
  **Success criteria**: A fresh instrumentation run on a new branch includes `gen_ai.usage.output_tokens`/`gen_ai.usage.input_tokens` without a follow-up manual fix, confirmed via COV-005 enforcement (blocking, not advisory) rather than by manual inspection alone.
