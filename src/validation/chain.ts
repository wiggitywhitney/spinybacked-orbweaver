...
export async function validateFile(input: ValidateFileInput): Promise<ValidationResult> {
  ...
  // 3. Lint checking (diff-based) — dispatched through provider. Passes the
  // real filePath (matching checkSyntax() above) so each provider can resolve
  // linter config from the file's own directory, not process.cwd() — these
  // differ when instrumenting a target project from outside its directory.
  const lintResult = await provider.lintCheck(originalCode, instrumentedCode, filePath);
