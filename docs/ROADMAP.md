...
- Watch: resolves.ts span/attribute naming not converging run-over-run in taze despite stable compilation ([issue #1071](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1071)) — new in run-17; compilation oscillation (#954/#958) is now fixed for 2 consecutive runs, but 4 of 6 span names and 1 attribute drifted vs. run-16; needs a third data point before treating as structural.
...
- SCH-003/SCH-004: count-cast pattern broadens to 5 files, near-synonym attribute in yarnWorkspaces.ts ([issue #1072](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1072)) — taze run-17; #1012's fix only addressed 2 files and didn't generalize the underlying judgment; 2 of 5 files now show a "disguised" recurrence where the schema was retyped to `string` to match the cast instead of the cast being removed.
- CDQ-007: unsanitized absolute filesystem paths regress in 6 of 13 taze files ([issue #1073](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1073)) — taze run-17; largest new finding by file count; `resolves.ts` already sanitizes correctly and is the reference pattern for the fix.
- IS SPA-002: orphan span confirmed recurring in taze `resolves.ts` across run-16 and run-17 ([issue #1008](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1008)) — promoted from watch to real fix candidate; same async-boundary context-loss shape across 2 consecutive runs; fix belongs in spiny-orb's context propagation, not the eval target.
...
- `LanguageProvider.lintCheck()` has no way to receive the real project directory for config resolution ([issue #1069](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1069)) — falls back to `process.cwd()`, which only resolves the correct `.prettierrc`/`pyproject.toml`/`ruff.toml` when spiny-orb is invoked from the project root; found via CodeRabbit review during PRD #373's Python provider work but affects all three providers (JS/TS/Python).
