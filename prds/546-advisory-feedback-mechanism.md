...
- [ ] **M3 — Add advisory-only pass for files that pass with advisory findings**: After the blocking retry cycle exits with `validation.passed === true` AND `validation.advisoryFindings.length > 0`, add a post-loop block in `instrument-with-retry.ts` that fires one additional instrumentation attempt targeting the advisory findings. Implementation specifics (Decision 5, Decision 6):
