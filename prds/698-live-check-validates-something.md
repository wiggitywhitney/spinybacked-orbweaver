...
| 2026-05-05 | M3 init file uses `NodeTracerProvider` (not `NodeSDK`) + explicit `OTLPTraceExporter` import | The PRD and research doc specified NodeSDK with `OTEL_EXPORTER_OTLP_PROTOCOL=grpc`. Changed to `NodeTracerProvider` from `@opentelemetry/sdk-trace-node` + `OTLPTraceExporter` from `@opentelemetry/exporter-trace-otlp-grpc` to enable direct double-init detection: `NodeTracerProvider` lets us call `trace.setGlobalTracerProvider(provider)` before `.start()` and capture the return value directly. `NodeSDK.start()` calls `setGlobalTracerProvider` internally with no way to inspect the return value without monkeypatching. Both `@opentelemetry/sdk-trace-node` and `@opentelemetry/exporter-trace-otlp-grpc` are transitive deps of `@opentelemetry/sdk-node` and available in the target project's `node_modules` whenever sdk-node is. The `OTEL_EXPORTER_OTLP_ENDPOINT` env var is still read by `OTLPTraceExporter` automatically — no change to the coordinator's env injection. |
