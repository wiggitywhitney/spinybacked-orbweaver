...
- [ ] **M2 — Add empty schema prerequisite gate with offer to add OTel semconv**: Per the Decision Log, implement this as a **hard block**. Start by reading OD-1 in the Outstanding Decisions section below — it contains a mandatory research step that must complete before you write any code for the offer flow. Then: in `src/config/prerequisites.ts`, locate `checkWeaverSchema()` (around line 183). After the `weaver registry check` call succeeds, resolve the schema using `resolveSchema` from `src/coordinator/dispatch.ts` (the same function used by dispatch — read that file to find the correct import and call signature), count registered attributes via `extractAttributeNames`, and if the count is zero: (a) in CLI interactive mode, prompt "No registered attributes found. Add OpenTelemetry semantic conventions as a registry dependency? [y/N]"; if the user accepts, implement the addition per OD-1's findings, then re-resolve and re-count; if the count is still zero, or if the user declines, add a clear error to the `PrerequisiteCheckResult` errors array: "No registered attributes found in schema at `{schemaPath}`. The agent has no naming patterns to follow and cannot run. Add OTel semantic conventions as a Weaver registry dependency: https://opentelemetry.io/docs/specs/semconv/"; (b) in non-interactive mode (MCP, `--yes` flag), add the error and return immediately without prompting. To thread interactive mode into `checkWeaverSchema`, add an `options?: { interactive?: boolean }` parameter — grep for other callers of `checkWeaverSchema` and update them. Do NOT change the behavior for schemas that already have registered attributes — only the zero-attribute path is new. Write unit tests: (1) empty schema + non-interactive → errors array contains the message, run blocked; (2) empty schema + interactive + user accepts + OD-1 addition succeeds → run proceeds; (3) empty schema + interactive + user declines → errors array contains the message, run blocked.
