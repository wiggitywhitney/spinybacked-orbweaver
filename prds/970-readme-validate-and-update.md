...
**Acceptance criteria:**
- [x] Root-level inventory produced and reviewed with Whitney
- [x] Approved cleanup actions implemented
- [x] No files deleted or moved without explicit Whitney approval
- [x] GitHub home page shows a cleaner file list after changes
...
**[2026-07-05] M6 root-level cleanup: FETCH_HEAD deleted outright, audit-findings/ moved rather than deleted**: M6's Step 2 proposed a symmetric treatment for `.DS_Store`, `FETCH_HEAD`, and `audit-findings/` (untrack/gitignore or move). Whitney asked whether `FETCH_HEAD` and `audit-findings/` should instead be deleted outright. A `git grep` check found zero references to `FETCH_HEAD` anywhere in the tracked codebase — it is a stray git-internal artifact with no historical or functional value, so it was deleted outright (`git rm FETCH_HEAD`) rather than merely untracked. `audit-findings/` was found to be referenced by relative path in `PROGRESS.md` and three completed PRDs (`prds/done/845-nds003-content-aware-diff.md`, `prds/done/857-validation-infrastructure-audit.md`, `prds/done/875-nds003-ast-comparison.md`) as a record of past validation-rule audits, so it was moved to `docs/audit-findings/` (`git mv`) rather than deleted, preserving the content and its git history. The `research/` directory (also approved for cleanup) was similarly moved to `docs/research/`, merging with an existing `docs/research/` directory produced by the `/research` skill — `git mv research docs/research` initially nested one level too deep (`docs/research/research/`) because the destination already existed; corrected by moving the two files up one level and removing the empty nested directory. **Why**: `.gitignore`-worthy artifacts (regenerable, like `.DS_Store`) get untracked; zero-reference cruft gets deleted; content with ongoing reference value gets moved, never deleted, regardless of how "stray" it initially looks. **Impact**: stale relative-path references to `audit-findings/...` remain unchanged inside `PROGRESS.md` and the three closed PRDs listed above — these are narrative history of already-completed work, not active broken links, and rewriting them was judged out of scope for M6. Future `git mv` of a directory into an existing same-named destination directory should be expected to nest one level deeper than intended; verify with `ls` after the move. **Code Impact**: `.gitignore` (added `.DS_Store`), `FETCH_HEAD` (deleted), `audit-findings/` → `docs/audit-findings/` (renamed, 7 files), `research/` → `docs/research/` (renamed, 2 files, merged into existing directory). **Owner**: Whitney (approved the revised asymmetric plan in this session).
