...
**Step 0**: Read the M1 Decision Log entry for "metric-to-trace linking" before starting this milestone. M1 must be complete. Also read the "M2 blocker identified" Decision Log row and its linked research (`docs/research/datadog-metrics-without-limits-tag-configuration.md`) — `traces.span.metrics.duration` and `commit_story.llm.output_tokens` are known to return zero queryable tags due to a Datadog Metric Tag Configuration (Metrics without Limits™) gap, not a Collector or code issue. If that row still says "requires Whitney's explicit approval" and no approval has been recorded, stop and get her approval before applying the tag-configuration fix in Datadog — do not attempt to work around it in code or the Weaver schema.

**Step 1**: Using the Datadog MCP (`search_datadog_metrics` or `get_datadog_metric`), validate each of the following queries produces data. Expect the Duration and Token cost queries below to fail to group by their listed tags until the M2-blocker fix (above) has been applied and approved — this is not a new bug, it's the tracked blocker resurfacing.
