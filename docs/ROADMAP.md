...
- Advisory rule feedback mechanism ([PRD #546](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/546)) — wire advisory findings into the fix loop so the agent is directed to address them; currently every advisory rule produces dead signal because the fix prompt says "Fix ONLY the failing rules."
- COV-004 `process.exit()` exemption + RST-006 ([PRD #553](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/553)) — exempt uninstrumentable functions from span coverage requirement and add a safety-net rule; fixes the root cause of the `index.js` acceptance gate cascade.
