...
- [ ] SCH-002's extension acceptance path calls `checkSemanticDuplicate` from `semantic-dedup.ts` with `useJaccard: true`; `inferredType` is **absent** at extension-acceptance time (no value AST node — extensions are just key strings). The type-compat and namespace pre-filters are still applied by `checkSemanticDuplicate` when `inferredType` is provided (see the "not in registry" path below).
- [ ] SCH-004's `inferValueType`, type compatibility pre-filter, namespace pre-filter, and post-validation patterns migrated to SCH-002 (sourced from `src/languages/javascript/rules/sch004.ts` — the canonical copy with full logic, not the stale `tier2/` copy). `inferValueType` takes a ts-morph `Node` — copy it into SCH-002 directly (do NOT put it in `semantic-dedup.ts`, which has no ts-morph dependency). `checkAttributeKeysMatchRegistry` must extend `extractAttributeKeys` to also return the inferred value type so the "not in registry" path can pass `inferredType` to `checkSemanticDuplicate`.
