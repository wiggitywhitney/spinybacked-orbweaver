...
- Bump spiny-orb to 2.0.0 and document release-time version-classification process ([issue #1048](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1048)) — the published npm version has stayed at 1.0.0 despite a breaking `--thinking` flag change (PRD #752) and months of unreleased work in PROGRESS.md; no version has ever been cut to reflect it.
- publish.yml missing Weaver CLI install — v2.0.0 npm publish failed ([issue #1052](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1052)) — blocks #1048's release; `npm publish`'s `prepublishOnly` test run fails in CI because `publish.yml` never installs Weaver, unlike `npm-release-test.yml` and `acceptance-gate.yml`.
