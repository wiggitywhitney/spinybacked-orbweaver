...
describe.skipIf(!API_KEY_AVAILABLE)('Acceptance Gate — Phase 3 Fix Loop', () => {
  ...
  describe('successful instrumentation through fix loop', () => {
    it('instruments user-routes.js and produces a fully populated FileResult', { timeout: 180_000 }, async () => {
    ...
    it('instruments order-service.js with error handling preserved', { timeout: 180_000 }, async () => {
  ...
  describe('validation strategy reflects actual attempt used', () => {
    it('reports the correct strategy in FileResult', { timeout: 180_000 }, async () => {
      const { filePath, originalCode } = setupTempFile('src/user-routes.js');
      const config = makeConfig();
