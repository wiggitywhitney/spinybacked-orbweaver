...
| 2026-03-06 | API-002 enforced at write-time, no post-hoc Tier 2 checker | The coordinator's `installDependencies` writes `@opentelemetry/api` as `--save-peer` (libraries) or `--save` (applications) based on `dependencyStrategy`. This enforces API-002 at write-time rather than validating after the fact. A post-hoc Tier 2 checker that re-reads `package.json` would be redundant — the coordinator itself is the enforcement mechanism. This is a deliberate deviation from the PRD's framing of API-002 as a validation chain stage: since the coordinator controls dependency installation directly, re-verifying the outcome provides no additional safety. |
