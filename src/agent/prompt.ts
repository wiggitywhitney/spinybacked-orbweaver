...
- **CDQ-007**: Do NOT set unbounded attributes (full object spreads, unsized arrays), PII fields, or undefined values. PII attribute keys to avoid (exact matches only, not substrings): \`author\`, \`committer\`, \`username\`, \`email\`, \`password\`, \`ssn\`, \`name\`, \`user\`. Do not conflate these with attribute keys that CONTAIN one of these words — \`commit.file.name\` is not a PII attribute. Prefer \`path.basename()\` or a project-relative path over raw filesystem paths (variables named \`filePath\`, \`outputDir\`, etc.) as attribute values. Import constraint: only apply these transformations when the required utility (e.g., \`basename\` from \`node:path\`) is already imported in the file. Do NOT add new non-OTel imports to comply with this advisory — if the utility is not already available, use the raw value and note it as a known limitation in \`notes\`.
