...
- Span Category Breakdown table reports unverified LLM self-report as fact, silently drops files ([issue #1042](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1042)) — run-26 found the table presents unverified `spanCategories` self-report as measured fact and silently omits files where it comes back null; worked as part of a 4-issue PR-report trust batch on branch `report-fixes-and-prd-1038`.
- Advisory Findings section drops line numbers and double-prints rule names/descriptions ([issue #1043](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1043)) — `pr-summary.ts`'s Advisory Findings renderer never surfaces `lineNumber` (unlike `reasoning-report.ts`, which does) and duplicates rule ID/name text; same 4-issue batch as #1042.
- Live-check compliance report is never committed; PR links to a file that doesn't exist in the repo ([issue #1044](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1044)) — the PR body references `spiny-orb-live-check-report.json` as if it's part of the repo, but it's only written locally and never staged for commit; same 4-issue batch as #1042.
- PR body formatting: raw H1 breaks heading hierarchy, stray blank lines, unhelpful flat span-ID list ([issue #1045](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1045)) — raw Weaver schema-diff text embeds its own H1 that outranks the surrounding section, plus redundant flat span-ID dump; same 4-issue batch as #1042.
