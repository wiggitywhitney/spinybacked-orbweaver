...
| 2026-03-04 | `'max'` effort level intentionally omitted from `AgentEffort` config | The SDK accepts `'low' | 'medium' | 'high' | 'max'` for `OutputConfig.effort`, but the spec doesn't reference `'max'`. Omitted to keep the config surface aligned with spec. Adding it is a one-line schema change if needed. |
