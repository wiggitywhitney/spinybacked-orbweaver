...
- [x] **Milestone 9: DX verification** — Error messages are self-explanatory for AI intermediary consumption. Progress output is semantically meaningful. Cost ceiling displayed in dollars before processing. All failure scenarios produce actionable messages. Verify: trigger each failure mode (no config, invalid path, agent failure, schema checkpoint failure, budget exceeded) and confirm error output includes what failed, why, and what to do.

- [x] **Milestone 10: README and usage documentation** — Write README.md and usage documentation using the `/write-docs` skill (validates all examples by executing real commands). README covers: what this tool does, installation, quick start with `orb init` + `orb instrument`, configuration reference, MCP integration with Claude Code, GitHub Action setup. All command examples must come from real execution against the test fixture project — no invented output. Verify: (a) README renders correctly, (b) all command examples are validated, (c) installation instructions work from a clean clone. Note: All examples verified against real CLI/MCP execution per issue #46. Fixed `--no-pr` yargs bug discovered during verification.

- [x] **Milestone 11: Resolve commit-story local dependency** — Remove `"commit-story": "file:../commit-story-v2"` from `package.json` devDependencies so `npm install` works on a clean clone. Whitney installs it locally via `npm link commit-story` (symlink in `node_modules/`, already gitignored). The journal output directories `.commit-story/` and `journal/` are already gitignored. No code changes needed — commit-story hooks detect availability at runtime and degrade gracefully when absent. Verify: (a) `npm install` succeeds from a clean clone without the sibling directory, (b) `npm link commit-story` locally restores journal integration, (c) journal entries still work after the change.
...
- [x] **Milestone 15: GitHub Action verification workflow** — Separate CI workflow (`.github/workflows/verify-action.yml`) that exercises the composite `action.yml` commands against the test fixture project. Replicates the action's exact commands (npm ci, Weaver install, `orb instrument --yes --output json`, result parsing) rather than `uses: ./` (see Decision Log). Gated behind `run-acceptance` label (costs API money). Verify: (a) workflow runs successfully against `test/fixtures/project/` with proper setup (package.json, orb.yaml, semconv, git init), (b) step outputs `result` (valid JSON) and `summary` (human-readable string) are populated, (c) workflow is valid YAML with `run-acceptance` label gate.
...
| 2026-03-11 | **Keep commit-story locally via `npm link`, not in package.json** — Remove `file:` devDependency but keep local journal integration via `npm link commit-story`. Symlink lives in `node_modules/` (gitignored), journal directories already gitignored. No code changes — commit-story hooks degrade gracefully when absent. | Whitney uses commit-story journal entries for PRD progress tracking and wants to keep them. The `file:../commit-story-v2` path dependency is the only thing breaking clean clones — removing it from package.json while using `npm link` preserves the workflow locally without affecting other cloners. |
...
| 2026-03-08 | **Acceptance gate: label-based PR trigger + weekly cron, advisory only** — Acceptance tests run on PRs labeled `run-acceptance` (feature PRs), weekly cron on main (safety net), and manual `workflow_dispatch`. Not merge-blocking. Quick-fix PRs skip acceptance tests entirely. | LLM acceptance tests take ~28 min and cost real API money. Running on every PR wastes resources on typo fixes. Label-based triggering is industry standard for expensive test suites (research: Promptfoo CI/CD, GitHub community patterns). Weekly cron catches regressions missed by unlabeled PRs. `contains()` syntax used over `github.event.label.name` per GitHub community guidance (reliable across all PR event types). |

| 2026-03-11 | **Use `.env` file for API key during README verification, not vals** — `ANTHROPIC_API_KEY` is stored in `.env` (gitignored) to replicate the real end-user experience. Future CLI and MCP testing commands should source the key from `.env` (`set -a && source .env && set +a`), not `vals exec`. This avoids `vals exec` PATH stripping and environment quirks that mask real user issues. | `vals exec` strips inherited env vars (including `ANTHROPIC_BASE_URL` and `PATH`), which accidentally avoids bugs a real user would hit. Testing with `.env` replicates what users actually do. |
| 2026-03-11 | **`ANTHROPIC_BASE_URL` must be unset for direct API testing** — Whitney's environment has `ANTHROPIC_BASE_URL` set to a Datadog AI gateway proxy that requires a `source` header the Anthropic SDK doesn't send. Any direct CLI or MCP testing (not via `vals exec`) will fail with "Missing required header: source". Fix: `unset ANTHROPIC_BASE_URL` before testing, or add `"ANTHROPIC_BASE_URL": ""` to the MCP server `env` block in `.mcp.json`. `vals exec` accidentally avoids this because it strips inherited env vars. | Discovered during MCP `instrument` end-to-end testing. This is environment-specific but affects any user with an AI gateway proxy on `ANTHROPIC_BASE_URL`. |
| 2026-03-11 | **MCP setup uses `.mcp.json` with env var expansion, not hardcoded keys** — README MCP Setup section updated to show `.mcp.json` (project-level config) with `"ANTHROPIC_API_KEY": "${ANTHROPIC_API_KEY}"` syntax. Claude Code and other MCP clients expand env vars at startup. | `.claude/settings.json` works too but `.mcp.json` is the standard project-level config location for MCP servers. Env var expansion avoids committing secrets. |
| 2026-03-11 | **Port 4317/4320 conflict filed as issue #55** — Weaver `registry live-check` uses ports 4317 (gRPC) and 4320 (admin) by default, conflicting with standard OTel collector infrastructure. Weaver supports `--otlp-grpc-port` and `--admin-port` flags; the codebase has configurable `LiveCheckOptions`. Fix: use non-default ports and downgrade `orb init` port check from blocking to advisory. | `orb init` currently exits with code 1 if ports are occupied, blocking setup for anyone running an OTel collector or Datadog agent. This will be fixed on a separate branch before README verification resumes. |
| 2026-03-11 | **`--yes` flag suppresses both cost ceiling display AND confirmation** — The coordinator only fires `onCostCeilingReady` when `config.confirmEstimate` is true. Since `--yes` sets `confirmEstimate: false`, the cost ceiling is never calculated or displayed — not just the prompt. README flag description updated to "Skip cost ceiling display and confirmation". | Discovered during real CLI testing. The previous description ("Skip cost ceiling confirmation") was misleading — users wouldn't see the cost at all with `--yes`. |
| 2026-03-11 | **`--no-pr` yargs bug fix: define as `--pr` with negation** — The original `.option('no-pr', ...)` definition caused yargs strict mode to reject the flag as "Unknown argument: pr" because yargs interprets `--no-X` as negating `--X`. Fixed by defining `.option('pr', { default: true })` so `--no-pr` becomes the standard yargs negation form. | Discovered during real CLI testing for issue #46. The bug meant `--no-pr` was documented but non-functional. All 1190 tests pass after the fix. |
| 2026-03-11 | **Verify action.yml via command replication, not `uses: ./`** — The verify-action workflow replicates the action's exact commands (`npm ci`, Weaver install, `orb instrument --yes --output json`, result parsing) against a test fixture project rather than using `uses: ./`. Composite actions run with CWD=workflow workspace (the orbweaver repo), making it impossible to use `uses: ./` against a separate test project without polluting the repo root with test orb.yaml and semconv. Replicating commands verifies the same things: Weaver binary install on Ubuntu, CLI JSON output shape, and result-parsing Node.js one-liner. | `uses: ./` would require creating orb.yaml and semconv/ in the repo root, conflicting with the repo's own structure. The action's value is in the commands it runs, not the composite YAML wiring. |
