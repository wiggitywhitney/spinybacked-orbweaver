...
### Surprises & Gotchas
- The multi-instance tracing-break risk that grounds JS API-002 is an npm-specific artifact of nested `node_modules` (multiple copies of `@opentelemetry/sdk-*` can coexist at different tree depths with independently duplicated global state). Python's flat `site-packages` model and pip's resolver make this far less likely under normal installs — pip raises `ResolutionImpossible` rather than installing two conflicting versions side by side. This is not an absolute guarantee: vendoring, custom import paths, and import hooks (`sys.meta_path`, manual `sys.modules` injection) can still expose multiple copies or module objects outside pip's normal resolution path.
...
**Interpretation:** npm's API-002 risk exists *because* nested `node_modules` lets multiple copies of `@opentelemetry/sdk-*` coexist at different tree depths, each with independent module-level global state (context/propagation globals duplicated per copy). Python's `site-packages` has no equivalent nesting — under a normal pip-managed install, one `opentelemetry-sdk` module object per interpreter process is cached in `sys.modules`, so there is no ordinary mechanism for two independent copies of the global `_TRACER_PROVIDER` singleton to exist simultaneously and silently diverge the way duplicate npm SDK copies can. Vendoring or custom import hooks could still construct such a scenario, but that is an unusual, self-inflicted packaging choice rather than the default behavior pip's resolver produces.
...
### Recommendation
Keep Milestone D4's rule (advisory, checks `opentelemetry-api` vs `opentelemetry-sdk`/exporter/instrumentation placement in Python manifests) — it is still good package hygiene and matches the real OTel Python community convention. But rewrite D4's rationale: don't claim it prevents "silent trace-propagation breakage" the way JS API-002's rationale does, since that specific risk doesn't exist in Python under normal packaging. The Python-specific motivations are: (a) forcing an SDK/exporter choice on downstream consumers who may want a different backend, (b) unnecessary transitive dependency bloat (exporter deps, gRPC, etc.) pulled into libraries that don't need them. A manifest-level dependency on `opentelemetry-sdk` does not itself configure providers or trigger double-init warnings — that is a runtime concern, not a packaging-placement concern, so it does not belong in D4's rationale.
