...
const RULE_HUMAN_DESCRIPTIONS: Partial<Record<string, string>> = {
  ...
  'COV-004': 'Fired because this async function doesn\'t have ' +
  ...
  'COV-005': 'Fired because one or more spans are missing ' +
  ...
  'CDQ-006': 'Fired because span.setAttribute() is called with an ' +
  ...
  'CDQ-007': 'Fired for one or more of: a PII attribute name ' +
    ...
    'basename() when already imported, or inline filePath.split(/[\\\\/]/).pop() otherwise — no new ' +
    'import required either way.',
  ...
  'CDQ-010': 'Fired because a string-only method ' +
  ...
  'RST-001': 'Fired because a span was added to a short, ' +
  ...
  'RST-002': 'Fired because a span was added to a getter ' +
  ...
  'RST-003': 'Fired because this function immediately ' +
  ...
  'RST-004': 'Fired because a span was added to an ' +
  ...
  'RST-005': 'Fired because the agent added spans to a ' +
  ...
  'RST-006': 'Fired because the agent added a span to a ' +
  ...
  'API-002': 'Fired because @opentelemetry/api is in the ' +
  ...
  'NDS-001': 'Fired because the instrumented file is no longer valid ' +
    'JavaScript/TypeScript — the agent introduced a syntax error. The file was reverted. The ' +
    'agent will retry.',

  'NDS-002': 'Fired because the agent\'s changes caused one or more ' +
  ...
  'NDS-003': 'Fired because the agent modified, removed, or reordered ' +
  ...
  'NDS-004': 'Fired because the agent changed the parameters or ' +
  ...
  'NDS-005': 'Fired because the agent removed or restructured ' +
  ...
  'NDS-006': 'Fired because the agent used the wrong import syntax ' +
    'for this project. An ESM project got require(), or a CommonJS project got import/export. ' +
    'The file was reverted.',

  'NDS-007': 'Fired because the agent added error recording ' +
  ...
  'NDS-008': 'Fired because the generated code contains a ' +
  ...
  'COV-001': 'Fired because an entry point — a route handler, CLI ' +
  ...
  'COV-002': 'Fired because an outbound call (database query, HTTP ' +
  ...
  'COV-003': 'Fired because a span is missing error recording in a ' +
  ...
  'API-004': 'Fired because the agent imported from ' +
  ...
  'SCH-001': 'Fired because a span name doesn\'t match ' +
  ...
  'SCH-002': 'Fired because an attribute key used in ' +
  ...
  'SCH-003': 'Fired because an attribute value doesn\'t match ' +
  ...
  'CDQ-005': 'Fired because the agent used tracer.startSpan() ' +
...
/**
 * List every rule ID that has a registered human-facing description.
 * Intended for tests that need to audit all descriptions generically
 * (e.g. checking none of them restate their own rule ID/name).
 *
 * @returns Array of rule IDs with a registered RULE_HUMAN_DESCRIPTIONS entry
 */
export function getRuleIdsWithHumanDescriptions(): string[] {
  return Object.keys(RULE_HUMAN_DESCRIPTIONS);
}
