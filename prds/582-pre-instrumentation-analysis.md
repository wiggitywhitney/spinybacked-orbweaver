...
- [ ] **M8 — Acceptance gate diagnostic infrastructure + CLAUDE.md rule (Decision D-12)**: Before the CLAUDE.md rule can be written, the tooling must exist to support all five dimensions. Two code changes:

  **Dimension 2 — Actual instrumented code**: In `test/commit-story-v2/acceptance-gate.test.ts`, update `dumpDiagnostics` to accept the `filePath` as a second argument and write the full instrumented file to a stable path — `/tmp/spiny-orb-debug-{label}.js` — on every run, then log that path. Reading the file must happen after `instrumentWithRetry` completes and before `afterEach` removes the temp dir. Log the path so future sessions know where to find it: `console.log('[${label} instrumented file] /tmp/spiny-orb-debug-${label}.js')`. The file persists across the test run, letting any session inspect exactly what the agent generated, not just what the validator reported.

  **Dimension 5 — Agent thinking**: In `src/agent/instrument-file.ts`, extract thinking blocks from `response.content` (filter for `b.type === 'thinking'`) after `stream.finalMessage()`. Add an optional `thinkingBlocks?: string[]` field to `InstrumentationOutput` in `src/agent/schema.ts`. Thread through `FileResult` in `src/fix-loop/types.ts`. Log the first thinking block (first 500 chars) in `dumpDiagnostics`. Write unit tests verifying the extraction.

  **CLAUDE.md rule**: Once both dimensions are available in the diagnostic output, add to `spinybacked-orbweaver/.claude/CLAUDE.md` under "Acceptance Gate Tests": "When an acceptance gate test fails, examine all five dimensions before diagnosing or making code changes: (1) history — when did this test last pass in CI and what changed?; (2) actual instrumented code — read what the agent generated, not just the error codes; (3) error messages from the validator — including block previews in NDS-005 messages; (4) agent notes — what the agent reported about its own decisions; (5) agent thinking — the first attempt's reasoning. Do NOT diagnose from error codes alone."
