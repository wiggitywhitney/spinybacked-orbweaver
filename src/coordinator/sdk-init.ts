...
export async function updateSdkInitFile(
  ...
  const fallbackContent = generateFallbackFile(uniqueLibraries);
...
function tryUpdateNodeSdkPattern(
  ...
  // Determine which libraries are already present
  ...
  const newLibraries = libraries.filter(
    lib => !existingInstrumentations.has(lib.importName) && !existingImports.has(lib.importName),
  );

  if (newLibraries.length === 0) {
    return sourceText; // All already present
  }
  ...
  // Add import/require statements for new libraries
  const importStatements = newLibraries.map(lib => {
    if (esm) {
 ...
 */
function generateFallbackFile(libraries: LibraryRequirement[]): string {
