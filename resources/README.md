...
- **Why vendored instead of fetched at install/build time:** `tree-sitter-python`'s npm package does contain a usable `.wasm` file (byte-identical to the GitHub Release asset), but installing it still runs the package's `node-gyp-build` install script — see `docs/research/web-tree-sitter-python-parser.md`. Downloading the `.wasm` from GitHub Releases directly skips that install script entirely rather than depending on a matching prebuild for the current platform. A `postinstall` fetch script would add a network dependency to every install and a new failure mode if the release asset is renamed or removed. Vendoring keeps the parser's provenance pinned to the exact bytes that were ABI-verified above.
