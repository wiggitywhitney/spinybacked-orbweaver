...
- [ ] **M4b — Fix remaining `index.js` acceptance gate failures (prerequisite for PRD closure)** (Decision D-8): Smoke test run 2026-04-27 revealed one persistent failure after M1–M3: NDS-005 (Control Flow Preserved) fires once per attempt because the inner try/catch at ~line 489 of `index.js` (wraps `triggerAutoSummaries`, no rethrow) is dropped when `main()` is wrapped in `startActiveSpan`.
  ...
  **If still failing — 5-dimension diagnosis before any further changes**: Before making additional code changes, collect all available diagnostic data: (1) check when the test last passed in CI git history; (2) capture the actual instrumented code (read `filePath` in the test before `afterEach` cleanup); (3) read the full error messages including the NDS-005 block preview; (4) read the agent `notes`; (5) if thinking blocks are available, read the first attempt's reasoning. Do NOT assume what the agent did from error codes alone — the notes on 2026-04-27 showed the agent *thought* it preserved the try/catch when error codes suggested otherwise.
