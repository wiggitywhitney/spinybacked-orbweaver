...
- summary-graph.js acceptance gate: SCH-002 same-pass check causes partial instead of success ([issue #1063](https://github.com/wiggitywhitney/spinybacked-orbweaver/issues/1063)) — first observed failure since the SCH-002 same-pass consistency check (#1056) merged; agent reused `entries_count` for two different concepts across daily/weekly/monthly node functions. Needs a repeat run to confirm reproducible vs. stochastic before deciding the fix.
