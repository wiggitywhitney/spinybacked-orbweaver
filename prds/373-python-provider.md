...
**Rationale (per Decision D-D4-1 — do not describe this rule as "identical to JS API-002"):** The API-vs-SDK convention itself is confirmed directly by OTel Python's own contrib guidance ([opentelemetry-python-contrib README](https://github.com/open-telemetry/opentelemetry-python-contrib/blob/main/README.md)): "Libraries that produce telemetry data should only depend on `opentelemetry-api`, and defer the choice of the SDK to the application developer." Unlike JS API-002, this rule is **not** motivated by a silent trace-propagation-break risk — Python's flat, single-version-per-environment `site-packages` model structurally prevents the multi-instance failure mode that motivates the JS rule (pip's resolver rejects conflicting versions rather than letting them coexist). The Python-specific motivation is package hygiene: (a) a library declaring `opentelemetry-sdk` forces an SDK/exporter choice on downstream consumers who may want a different backend, (b) it pulls unnecessary transitive dependency bloat (exporter deps, gRPC, etc.) into libraries that don't need them, (c) it risks "Attempting to instrument while already instrumented" double-init warnings when both a library and its consuming app independently configure providers.
