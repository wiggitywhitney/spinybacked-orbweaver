...
| 2026-06-02 | OQ-3 resolved: 0% truly dynamic keys; 27.9% multi-line literal false-negative rate is acceptable gap | Grepped `spiny-orb/instrument-1780313045724` in `wiggitywhitney/commit-story-v2` — 104 total `setAttribute` calls: 75 single-line literal (extracted), 29 multi-line literal with key on next line (not extracted — false negatives, not dynamic), 0 truly dynamic. Dynamic fraction = 0%, well below 20% threshold. Multi-line false-negative rate (27.9%) is a coverage gap but not a correctness risk — unextracted multi-line keys are still handled by the agent's explicit schemaExtensions declarations in the normal case; auto-registration only covers the gap when declarations are missing. |
...
- [ ] **M5 — Tests for end-to-end auto-registration**: Write an integration test that verifies the complete flow: agent uses an unregistered attribute in code without declaring it in `schemaExtensions` → auto-registration step runs → attribute appears in `agent-extensions.yaml` → SCH-002 validation passes. Use the existing acceptance gate test fixture infrastructure (see `test/acceptance-gate.test.ts` for fixture patterns). Verify that semantic duplicates are NOT auto-registered and SCH-002 still fires for them.

- [ ] **M6 — Update docs/rules-reference.md and agent prompt**: Per rules-related work conventions, read `docs/rules-reference.md` in full before making changes. Update the SCH-002 entry to document that novel attributes not declared in `schemaExtensions` are now auto-registered deterministically; include the known limitation that only single-line `setAttribute('key', value)` calls are extracted — multi-line calls where the key appears on a continuation line are not extracted and still rely on explicit agent declaration (OQ-3 finding, Decision Log 2026-06-02). Update `src/agent/prompt.ts` to clarify that `schemaExtensions` is now an intent signal rather than the only registration path — agents should still declare extensions to provide type, brief, and stability metadata, but omitting a declaration no longer causes a registration failure for single-line setAttribute calls. Run `/write-docs` to validate documentation changes.
