...
`setAttribute()` calls with dynamic keys (`span.setAttribute(keyVar, value)`) or template literals cannot be statically extracted. The extraction step should skip non-literal keys without error. Estimate what fraction of `setAttribute()` calls in real instrumented code use dynamic keys — if it's low, the gap is acceptable; if it's high, a separate mechanism may be needed.
