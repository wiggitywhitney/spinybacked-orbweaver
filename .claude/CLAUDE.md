...
**When a test fails, examine all five diagnostic dimensions before diagnosing or making code changes:** (1) history — run `gh run list --workflow=acceptance-gate.yml` (no limit) to see when it last passed and what changed — look back as far as needed to find the last green run; (2) actual instrumented code — read `/tmp/spiny-orb-debug-{label}` where `{label}` is the fixture file name passed to `dumpDiagnostics` (e.g., `index.js` → `/tmp/spiny-orb-debug-index.js`), written during every test run; (3) validator error messages — in the `dumpDiagnostics` console output, read `lastError`, which contains full NDS-005 block previews and complete messages for all blocking failures (not just the count summary in `errorProgression`); for oscillation failures, also read `lastErrorByAttempt`, which shows the full error text from each attempt so you can see whether the agent was converging or diverging; (4) agent notes — read `notes` in the same console output, what the agent reported about its own decisions; (5) agent thinking — read `thinkingBlocksByAttempt` in the same console output, one entry per attempt (oscillation failures need later attempts' reasoning, not just attempt 1). Do NOT diagnose from `errorProgression` summaries alone — they show only error counts, not the specifics needed to identify the root cause.
