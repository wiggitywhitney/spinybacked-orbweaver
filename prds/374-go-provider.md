...
- [ ] **OD-3 (tree-sitter-go):** Check `package.json` — is `web-tree-sitter` already a dependency (added in PRD #373)? If yes, use the existing `web-tree-sitter` integration. If no, run `/research tree-sitter-go` and adopt `web-tree-sitter` (WASM binding) consistent with the pattern established in PRD #373 OD-1. Do NOT use the native `tree-sitter` npm package — it requires Node 24 for v0.26. **Unverified as of 2026-09-17:** a quick check of the npm registry (`tree-sitter` latest is `0.25.1`, no `0.26.x` published, no `engines` field) and `node-tree-sitter`'s GitHub `main` branch (same: no `engines` field) found no corroborating evidence for this specific claim, but that is inconclusive rather than a disproof — the mandatory `/research tree-sitter-go` step below should re-verify this before it's relied on. **Sync/async reconciliation (PRD #373 Decision D-D1-2):** the `LanguageProvider` interface's AST methods (`findFunctions()`, etc.) are synchronous, but `web-tree-sitter`'s WASM setup is inherently async. `src/languages/python/ast.ts` resolves this with a **top-level `await`** at module load (`await Parser.init()`, `await Language.load(...)`) — ESM guarantees this resolves before any importer sees the module's exports. Reuse the same pattern for `src/languages/go/ast.ts` rather than re-deriving a different solution.
