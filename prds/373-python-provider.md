...
| OD-1 | tree-sitter-python via web-tree-sitter (npm v0.27.0), with the grammar `.wasm` sourced from tree-sitter-python's own GitHub Releases (npm v0.25.0) rather than its npm package | web-tree-sitter has no Node engine restriction, confirming it avoids native tree-sitter's Node 24 requirement; but the `tree-sitter-python` npm package ships a native Node addon (`node-gyp-build` install script), not a usable `.wasm` — its GitHub release publishes `tree-sitter-python.wasm` directly, avoiding both the native-build problem and an extra local CLI build step. ABI compatibility with web-tree-sitter 0.27.0 is expected but not yet confirmed at runtime: `tree-sitter-python` v0.25.0's own `devDependencies` target `tree-sitter-cli ^0.25.9` (ABI 15), which falls within web-tree-sitter 0.27.0's documented supported ABI range of [13, 15] — but this is based on published package metadata (🟡 medium confidence, third-party source), not an actual `Language.abiVersion` check. That runtime verification remains an open item in the D1 checklist below (`.wasm` sourcing-mechanism item) and must pass before this decision is fully load-bearing. Full findings: `docs/research/web-tree-sitter-python-parser.md`. | 2026-09-17 |
...
- [ ] **Decide the `.wasm` sourcing mechanism (not yet resolved by OD-1):** OD-1 confirmed *where* the grammar `.wasm` comes from (GitHub Releases, not the npm package) but not *how* the build/install process obtains and keeps it. **Recommendation (to be confirmed):** Vendor-and-commit — download `tree-sitter-python.wasm` from the pinned GitHub release once and commit it under a project-controlled path (e.g., `resources/tree-sitter-python.wasm`), rather than fetching it at install/build time via a `postinstall` script. A fetch script adds a network dependency to every install and a new failure mode (release asset renamed/removed); vendoring keeps the parser's provenance ABI-pinned to what was actually tested. Document the pinned version and source URL in a comment near the file. Decide before creating `ast.ts`, since the loading code in `ast.ts` needs a concrete file path to call `Language.load()` on. Record the decision in the Decision Log. Also verify ABI compatibility (`Language.abiVersion` after `Parser.init()`) against the actual pinned `web-tree-sitter` version at implementation time — see `docs/research/web-tree-sitter-python-parser.md`'s ABI compatibility note.
...
- [ ] `classifyFunction()` handles Python-specific entry point patterns: Flask/FastAPI decorator detection (required for v1, per OD-6), async handlers. Django view conventions are a stretch goal, not required for v1 — implement only if time permits after Flask/FastAPI coverage is complete.
