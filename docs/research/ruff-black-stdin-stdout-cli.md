...
For `formatCode(source, configDir)`/`lintCheck()`'s shared `runFormatter()`: try `ruff format -` first (spawn with `input: source`); fall back to `black -` the same way whenever Ruff doesn't produce output — whether Ruff isn't installed (`ENOENT`) or is installed but rejects this specific input (any other failure, including a real parse error). **Updated 2026-09-21** (a CodeRabbit finding on the CDQ-001 milestone caught the original recommendation above as stale): the original text said to fall back to Black *only* on `ENOENT`, reasoning that a Ruff parse-error failure (exit 2) would fail identically under Black — but that assumption doesn't always hold in practice (the two tools' parsers can diverge on edge cases), so `runFormatter()` now falls through to Black on any Ruff failure and only reports a failure once neither formatter has produced output. If Black's own attempt also throws `ENOENT`, return `source` unchanged. For `lintCheck()`, treat "neither tool installed" (both throw `ENOENT`) as the advisory failure with the canonical OD-2 message.
