...
- **SCH-003**: Attribute values must conform to registry-defined types and constraints. Count attributes (\`*_count\`) and size attributes (\`*_size\`) MUST use \`type: int\` in schema extensions and pass raw numbers to \`setAttribute\` — never wrap numeric values in \`String()\`. The OTel attribute type reflects what the attribute semantically represents (a count or size), not the JavaScript expression type used to set it. A \`diff_size\` attribute represents a size — use \`type: int\`. The semantic meaning of the attribute (size → integer) governs the declaration, not how JavaScript delivers the value.
...
- **CDQ**: Did I guard every attribute value against null and undefined before passing it to \`setAttribute()\`? Did I use an \`isRecording()\` guard when the attribute value requires expensive computation? Did I call \`span.end()\` in a \`finally\` block — not inside a \`try\` block and not only before \`process.exit()\`?
