...
**B2 — Two items deferred to PRD #372 (TypeScript provider prerequisites)**

These gaps were identified during `/write-prompt` review of `agent/prompt.ts`. They are harmless while JavaScript is the only provider, but will break correctness for any non-JS provider. **PRD #372 must address both before TypeScript instrumentation is wired.**
