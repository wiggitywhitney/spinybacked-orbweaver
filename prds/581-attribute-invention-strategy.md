...
- [ ] **M2 — Add empty schema prerequisite gate**: Per the Decision Log, implement this as a **warning that allows the user to proceed** (not a hard block) — the user may be intentionally starting with an empty schema and wanting to bootstrap it. In `src/config/prerequisites.ts`, locate `checkWeaverSchema()` (around line 183). After the `weaver registry check` call succeeds, resolve the schema using the same resolve path used in dispatch, count registered attributes, and if the count is zero emit a prerequisite warning (not failure) with this message: "No registered attributes found in schema at `{schemaPath}`. The agent cannot infer naming patterns from an empty registry — any invented attributes will be inconsistent with your project's conventions. Consider importing OTel semantic conventions as a Weaver registry dependency before running: https://opentelemetry.io/docs/specs/semconv/". Add the warning to the `PrerequisiteCheckResult` warnings array, not the errors array. Write a unit test confirming the warning fires on an empty resolved schema and does not block the run.
...
| 2026-04-23 | Empty schema gate is a warning, not a hard block | The user may be intentionally bootstrapping from an empty schema and wanting the agent to populate it. A hard block would prevent a valid use case. A warning with a clear remediation recommendation (import OTel semconv) preserves user agency while making the risk visible. |
