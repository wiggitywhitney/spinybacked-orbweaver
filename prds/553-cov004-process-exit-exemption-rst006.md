...
| 6 | Smoke test runs before full acceptance gate | Before triggering the ~55-minute acceptance gate CI run in M6, first run the targeted smoke test from M5 (a real API call against the process-exit fixture, ~3-5 minutes). The smoke test validates that the rule changes actually change agent behavior on the specific failure pattern. Only proceed to the full acceptance gate after the smoke test passes. This was a standing instruction confirmed by Whitney. | 2026-04-22 |
