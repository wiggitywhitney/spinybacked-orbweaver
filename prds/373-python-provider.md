...
- [ ] Create `src/languages/python/ast.ts` — function finding, import detection, export detection (Python has no explicit exports; use naming convention — public functions are those not prefixed with `_`), function classification, existing instrumentation detection. Parser setup: `await Parser.init()` once, then `await Language.load('resources/tree-sitter-python.wasm')` (the vendored file from D-D1-1 above — resolve the path relative to the package root, not `process.cwd()`, since spiny-orb runs against target projects whose cwd is not this package), then `parser.setLanguage(...)`. Call `.delete()` on `Parser`/`Tree`/`Query` objects when done — `web-tree-sitter` requires explicit cleanup (see `docs/research/web-tree-sitter-python-parser.md`'s memory management note).
