...
**SDK setup file exemption**: API-001 and API-004 evaluate application source files only. The SDK setup file (configured as `sdkInitFile` in spiny-orb.yaml, typically `src/instrumentation.js` or `src/instrumentation.ts`) is expected to import SDK packages and is exempt from these rules.
